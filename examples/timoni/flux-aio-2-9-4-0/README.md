...
From the repository root, run `node scripts/generate-timoni-flux-aio.mjs --generate` with Timoni 0.34.0 to repeat the static build. Run `npm run timoni-flux-aio:verify` to check the retained evidence offline and exercise tampering rejection. This fixture has not yet been admitted as a Catalog BaseVariantRecord.
