...
const receiptRoot = join(repoRoot, "runs", "nginx-use-more-now", "latest");

verifyUseMoreNow();
verifyFunctionScan();
verifySafeOps();

console.log("verified NGINX use-more-now scan and safe-ops receipts");

function verifyUseMoreNow() {
  const path = join(receiptRoot, "use-more-now-receipt.yaml");
  check(existsSync(path), "missing NGINX use-more-now receipt");
  const receipt = readYaml(path);
  check(receipt.kind === "UseMoreNowReceipt", "NGINX use-more-now receipt kind mismatch");
...
function verifyFunctionScan() {
  ...
  check(receipt.spec?.context?.space === "helm-nginx-use-more-now", "NGINX function scan receipt must target proof space");
...
function verifySafeOps() {
  ...
  check(receipt.spec?.context?.space === "helm-nginx-use-more-now", "NGINX safe-ops receipt must target proof space");
