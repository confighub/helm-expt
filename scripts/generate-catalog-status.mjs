...
const supportedCatalogEntries = {
  ...
  "grafana/grafana": {
    ...
    notes: [
      "Supported for local-test and proof-demo usage through real cub installer and ConfigHub receipts.",
      "existing-secret-ingress is the package default. It renders no Grafana admin Secret and is supported when admin Secret and ingress/TLS target facts are satisfied.",
      "static-passwords is kept as an explicit demo path. It ships a fixed, shared demo admin password baked into the rendered Secret (base64) — identical on every install and readable from the manifest.",
