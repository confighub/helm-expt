...
const receiptRoot = join(repoRoot, "runs", "redis-use-more-now", "latest");

verifyFunctionScan();
verifySafeOps();

console.log("verified Redis use-more-now scan and safe-ops receipts");
...
function verifyFunctionScan() {
  ...
  check(receipt.spec?.context?.space === "helm-redis-use-more-now", "function scan receipt must target Redis proof space");
...
function verifySafeOps() {
  ...
  check(receipt.spec?.context?.space === "helm-redis-use-more-now", "safe-ops receipt must target Redis proof space");
