  ...
  return {
    ...
    spec: {
      ...
      conclusion: result === "pass"
        ? "All three consumers pulled the same ConfigHub OCI bundle and the routed hook ran under each. OCI single-transport across Argo, Flux, and no-controller is proven."
        : result === "watch"
...
function summaryMd(r) {
  ...
  return `# OCI single-transport hook proof (Argo / Flux / cub-direct)
...
ConfigHub publishes \`${s.fixture}\` once to \`${s.ociHost}\`; each consumer pulls the **same** bundle on one throwaway cub-lk rig (kind + Argo + the OCI worker). Publish: **${s.run.publish}**${s.run.publishReason ? ` — ${s.run.publishReason}` : ""}.
...
<title>OCI single-transport hook proof — Argo / Flux / cub-direct</title>
<style>
...
<body><main>
  <h1>OCI single transport — does every controller run the routed hook from one bundle?</h1>
  ...
  <footer>One ConfigHub OCI bundle, three consumers (Argo / Flux / cub-direct). The OCI pull secret is copied/converted into flux-system and a temp oras config in memory — never printed or logged. Non-converged legs are <code>watch</code> with a reason.</footer>
</main></body></html>
`;
}
