...
function buildReport() {
  ...
  const rows = [
    ...
    axis("getHostByName", "not-scanned", "unknown", "unknown", "docs/reference/quirk-coverage.md", "This Helm function is not currently detected as its own axis.", "DNS lookups during template render would make render depend on the network environment.", "Add scanner detection and reject or require a captured fact binding."),
    axis("resource-policy-keep", "not-scanned", "unknown", "unknown", "docs/reference/quirk-coverage.md", "helm.sh/resource-policy: keep is not currently detected as its own axis.", "Uninstall and prune behavior may leave intentional orphans.", "Add rendered-object scan for resource-policy keep and route to lifecycle policy."),
    axis("post-renderer", "not-scanned", "unknown", "unknown", "docs/reference/customization-algorithm.md", "Post-renderers are handled by doctrine as explicit recipe stages or rejected, but not scanned from chart source.", "Final applied objects can differ from helm template output.", "Represent post-renderers as pinned recipe/function stages or mark unsupported."),
    axis("helm-version-branching", "not-scanned", "unknown", "unknown", "docs/reference/quirk-coverage.md", ".Capabilities.HelmVersion branching is not currently detected.", "Render output could vary by Helm binary version.", "Add scanner detection and bind Helm version into receipts."),
    axis("global-values", "not-scanned", "unknown", "unknown", "docs/reference/quirk-coverage.md", "global.* propagation is not currently surfaced as a high-density value axis.", "One value can affect many subcharts and objects.", "Add value-source-map support for global values and subchart import paths."),
  ];
...
function axis(axisName, coverageTier, top100Count, top500Count, evidence, currentTreatment, remainingGap, nextAction) {
  return {
    axis: axisName,
    coverage_tier: coverageTier,
    top100_count: String(top100Count),
    top500_count: String(top500Count),
