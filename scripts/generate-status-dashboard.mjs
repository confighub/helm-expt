...
function buildReport() {
  ...
  const top100PromotionFastTrackStorageRows = readCsv("data/top100-promotion-wave/fast-track-reviews/storage-rollback/storage-reviews.csv");
  ...
  rows.push(metric("top100", "useful-base proposal rows", usefulBasePipelineTotal, "", "partial", "data/useful-base-design-queue/queue.csv", "Total useful-base pipeline rows: remaining design proposals plus rows already realized in data/useful-base-realization-wave/wave.csv. Multiple proposed bases can exist for one chart; these are not supported bases until rendered and proved."));
  rows.push(metric("top100", "useful-base realized rows", usefulBaseRealizationRows.length, usefulBasePipelineTotal, "partial", "data/useful-base-realization-wave/wave.csv", "Useful-base proposals that now have recipe variants and cub installer package bases. These still need ConfigHub proof, live evidence, and production disposition."));
  ...
  rows.push(metric("top100", "useful-base proposals not yet built", usefulBaseRows.length, usefulBasePipelineTotal, "gap", "data/useful-base-design-queue/queue.csv", "Remaining design-queue rows where the proposed base shape still needs recipe/package artifacts and proof before catalog promotion."));
  ...
  rows.push(metric("proof lanes", "explicit lifecycle observation rows", passCount(baseRows, "lifecycle_observation"), baseRows.filter((row) => row.lifecycle_observation !== "missing").length, "good", "data/outcome-coverage/base-outcomes.csv", "Rows where CRD, webhook, controller-owned field, or post-apply API readiness was proven through an explicit lifecycle receipt rather than a plain bulk apply."));
  ...
  const draftSupportDecisionCount = count(productionSupportDecisionRows, "decision", "draft");
  rows.push(metric("production support decisions", "draft decision artifacts", draftSupportDecisionCount, productionRows.length, draftSupportDecisionCount === 0 ? "good" : "partial", "data/production-support-decisions/decisions.csv", "Draft support boundaries that still need a final target-scoped decision."));
  ...
  rows.push(metric("remote dependencies", "active P0 dependency closure work rows", remoteDependencyRows.filter((row) => row.priority === "P0" && row.workstream !== "keep-current").length, remoteDependencyRows.length, "gap", "data/remote-dependency-closure/top100.csv", "Highest-priority dependency closure rows needing import, lock, digest, or refresh-survival work. Keep-current rows are excluded."));
  ...
  const observedHookRows = hookRows.filter((row) => row.lifecycle_disposition === "lifecycle-observed").length;
  const partialHookRows = hookRows.filter((row) => row.lifecycle_disposition === "install-lifecycle-observed-upgrade-pending").length;
  rows.push(metric("hooks", "hook lifecycle observations present", observedHookRows, hookRows.length, observedHookRows === hookRows.length ? "good" : "gap", "data/hook-lifecycle/maintained-hook-queue.csv", "Maintained hook queue rows with runtime lifecycle observation or execution receipts."));
  rows.push(metric("hooks", "hook partial lifecycle observations present", partialHookRows, hookRows.length, partialHookRows === 0 ? "good" : "partial", "data/hook-lifecycle/maintained-hook-queue.csv", "Maintained hook queue rows whose fresh-install route has live observation while another hook phase remains pending."));
  ...
  const hookRoutesNeedingObservation = hookQueueRows.filter((row) => ["route-selected", "install-lifecycle-observed-upgrade-pending"].includes(row.status)).length;
  rows.push(metric("hooks", "hook routes still needing execution or observation", hookRoutesNeedingObservation, hookQueueRows.length, hookRoutesNeedingObservation === 0 ? "good" : "gap", "data/lifecycle-boundary/lifecycle-boundary.csv", "Hook-bearing rows with a selected route and at least one missing runtime observation or execution receipt."));
...
function hardProofGapWorkObjects(rows) {
  ...
  return [
    ...
    },
    {
      ...
      next_action: "Close dependency range policy, refresh-survival, and recipe-import gaps before stronger catalog claims.",
      source: "data/hard-proof-gaps/shortlist.csv",
...
function remoteDependencyWorkstreamObjects(rows) {
  ...
  return [
    ...
    },
    {
      section: "remote-dependency-closure-work",
      item_type: "workstream",
      item: "Backfill dependency provenance",
      count: counts.get("chart-lock-digest") ?? 0,
      next_action: "Record a Chart.lock digest or source-derived dependency provenance.",
      source: "data/remote-dependency-closure/top100.csv",
...
function top100WorkQueueObjects(top100Rows, workRows, usefulBaseRows, fastTrackRows) {
  ...
  return [
    ...
    },
    {
      ...
      next_action: "Open the storage/rollback reviews, choose the target boundaries, complete any proof lanes listed in fast-track.csv, then record target-scoped support decisions.",
  ...
  return [
    {
      ...
      item_type: "queue",
      item: "Hook candidate route plans",
    ...
    },
    {
      section: "hook-and-lifecycle-work",
      item_type: "queue",
      item: "Hook candidates not yet maintained",
      count: reviewedNotMaintainedRows.length,
      next_action: "Promote each candidate into a maintained lifecycle receipt, runtime observation path, or explicit blocker before support claims.",
