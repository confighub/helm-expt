...
function buildReport() {
  ...
  rows.push(metric("proof lanes", "top20 bases needing unresolved prerequisite or runtime review", top20BaseReadinessRows.filter((row) => ["target-prerequisite-needed", "runtime-watch", "runtime-review-needed", "hook-lifecycle-review-needed"].includes(row.user_readiness)).length, top20BaseReadinessRows.length, "partial", "data/top20-base-readiness/base-readiness.csv", "Base variants whose render parity is useful but whose target fit, runtime, or lifecycle behavior still needs review."));
