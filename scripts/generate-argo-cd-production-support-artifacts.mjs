...
const chart = "argo-cd/argo-cd";
const chartSlug = "argo-cd-argo-cd";
const version = "9.5.15";
...
const localObservationPath = join(repoRoot, "runs", "top20-local-kind", "argo-cd-default", "observation-receipt.json");
...
if (mode === "--generate") {
  ...
  writeYaml(join(outputRoot, "fresh-target-evidence-2026-06-09.yaml"), buildFreshEvidenceReceipt());
  console.log(`wrote Argo CD support artifacts -> ${relativeRepo(outputRoot)}/`);
} else if (mode === "--verify") {
  ...
  console.log("verified Argo CD support artifacts");
} else {
  console.log(`Usage:
  node scripts/generate-argo-cd-production-support-artifacts.mjs --generate
  node scripts/generate-argo-cd-production-support-artifacts.mjs --verify`);
}
  ...
  return {
    ...
    spec: {
      ...
      productionSupportUse: {
        ...
        claim: "The mutable rendered image references for this Argo CD base were resolved to registry manifest digests at the recorded time.",
...
function buildImagePolicyDecision() {
  ...
  return {
    ...
    spec: {
      ...
      claim:
        "The rendered Argo CD bases still contain mutable image tags, but each rendered image reference has digest-resolution evidence. For this public GitOps-controller support scope, the mutable tag is accepted as a target-scoped exception while stricter environments remain free to require digest-pinned bases or image overrides.",
      ...
      acceptedException: {
        reason:
          "The public proof target uses upstream Argo CD, Dex, and Redis image tags. The receipt set records the registry digest each tag resolved to at decision time, so reviewers can rerun the decision when upstream tags move.",
        operatorAction:
      ...
      evidence: [
        { path: relativeRepo(imageWorkdownPath), claim: "Summarizes Argo CD rendered image references." },
        { path: relativeRepo(imageReviewPath), claim: "Lists every rendered image reference found in the Argo CD object sets." },
        ...receipts.map((receipt) => ({
          path: receipt.path,
...
function buildSecurityDecision() {
  ...
  return {
    ...
    spec: {
      ...
      decidedAt: "2026-06-09",
      claim:
        "The recorded kube-linter findings are missing CPU and memory requests/limits on the Argo CD controller workloads. They are accepted for this public cub-lk GitOps-controller proof scope. Stricter environments should create a resource-policy base or overlay before support expansion.",
      ...
      acceptedFindings: [
        {
          ...
          disposition:
            "Accepted for the public cub-lk proof scope. Add CPU requests and limits in a hardened base for customer or production SLO scopes.",
        },
        {
          group: "unset-memory-requirements",
          disposition:
            "Accepted for the public cub-lk proof scope. Add memory requests and limits in a hardened base for customer or production SLO scopes.",
        },
      ],
      limits: [
      ...
      evidence: [
        { path: relativeRepo(scanWorkdownPath), claim: "Routes Argo CD scan findings to add-resource-policy." },
        { path: relativeRepo(externalScanPath), claim: "Records kube-linter warning counts for default and no-crds rendered object sets." },
...
function buildLifecycleDecision() {
  ...
  return {
    ...
    spec: {
      ...
      evidence: [
        { path: relativeRepo(localObservationPath), claim: "Local kind observation records CRD bootstrap, target Secret staging, and workload rollout." },
        { path: relativeRepo(kindParityPath), claim: "Two-cluster Helm-vs-installer parity stages target facts and passes semantic/runtime parity." },
...
function buildFreshEvidenceReceipt() {
  ...
  return {
    ...
    metadata: { name: `${chartSlug}-${supportedBase}-argo-oci-support-evidence-20260609` },
    spec: {
      chart,
      version,
      base: supportedBase,
      ...
      checks: [
        { name: "regular-helm-runtime", result: live.spec.legs.regularHelm.runtime.result, detail: "regular Helm workload runtime passed" },
        { name: "confighub-apply-runtime", result: live.spec.legs.configHubKubectlApply.runtime.result, detail: "ConfigHub rendered objects applied by kubectl reached runtime pass" },
      ...
      supportClaim: {
        ...
        detail: "Fresh target-scoped ConfigHub OCI and Argo evidence passed for the declared Argo CD default support scope.",
      },
      limits: [
        ...
        "Repository credentials, SSO, RBAC policy, app state backup/restore, and self-management are outside this base support claim.",
...
function verifyImageReceipt(variant) {
  ...
  check((receipt.spec.images ?? []).length === 3, `${receipt.path} expected three unique images`);
...
function verifyImagePolicyDecision() {
  ...
  check(decision.spec.decision === "mutable-image-exception-accepted-for-target-scope", "Argo CD image policy decision mismatch");
  check((decision.spec.variantsCovered ?? []).length === variants.length, "Argo CD image policy variants mismatch");
}

function verifySecurityDecision() {
  const decision = decisionFile("security-decision.yaml", "ProductionSecurityDecision");
  check(decision.spec.decision === "resource-policy-accepted-for-target-scope", "Argo CD security decision mismatch");
  check(decision.spec.findingSummary.totalFindings === Number(scanWorkdownRow().findingCount), "Argo CD security finding count mismatch");
  for (const row of externalScanRows()) {
    check(decision.spec.findingSummary.variants[row.variant].renderedObjectSetSHA256 === row.renderedSHA256, `Argo CD security rendered sha mismatch for ${row.variant}`);
  }
}

function verifyLifecycleDecision() {
  const decision = decisionFile("lifecycle-decision.yaml", "ProductionLifecycleDecision");
  check(decision.spec.decision === "lifecycle-observed-for-proof-scope", "Argo CD lifecycle decision mismatch");
  check(decision.spec.supportedBaseCandidate === supportedBase, "Argo CD lifecycle supported base mismatch");
  check(decision.spec.observedLifecycleSignals.localKindObservation.supportSecret.result === "pass", "Argo CD target Secret observation missing");
  check(decision.spec.observedLifecycleSignals.localKindObservation.crdsEstablished === 3, "Argo CD CRD observation count mismatch");
  check(decision.spec.observedLifecycleSignals.confighubOciArgo.argoSync === "Synced", "Argo CD lifecycle Argo sync mismatch");
  check((decision.spec.limits ?? []).some((item) => item.includes("does not prove zero-to-Argo self-bootstrap")), "Argo CD lifecycle decision must state bootstrap limit");
}

function verifyFreshEvidenceReceipt() {
  const receipt = decisionFile("fresh-target-evidence-2026-06-09.yaml", "ProductionSupportEvidenceReceipt");
  const live = liveReceipt();
  check(receipt.spec.supportClaim.state === "fresh-target-evidence-passed", "Argo CD fresh evidence claim mismatch");
  check(receipt.spec.observedAt === live.spec.observedAt, "Argo CD fresh evidence observedAt mismatch");
  check(receipt.spec.oci.revision === live.spec.legs.configHubOciArgo.ociRevision, "Argo CD fresh evidence OCI revision mismatch");
  check(receipt.spec.oci.sync === "Synced", "Argo CD Argo sync mismatch");
  check(receipt.spec.oci.health === "Healthy", "Argo CD Argo health mismatch");
}
...
function imageReceipt(variant) {
  ...
  check(existsSync(path), `missing ${relativeRepo(path)}; run npm run argo-cd:production-support`);
...
function imageWorkdownRow() {
  ...
  check(rows.length === 1, "expected one Argo CD image workdown row");
  return rows[0];
}
...
function externalScanRows() {
  ...
  check(rows.length === variants.length, "expected Argo CD external scan rows");
  return rows;
}
...
function scanWorkdownRow() {
  ...
  check(rows.length === 1, "expected one Argo CD scan workdown row");
  return rows[0];
}
...
function localObservation() {
  ...
  check(receipt.kind === "ObservationReceipt", `${relativeRepo(localObservationPath)} kind mismatch`);
  ...
  check(receipt.spec?.variant === supportedBase, `${relativeRepo(localObservationPath)} base mismatch`);
