...
const receiptRoot = join(repoRoot, "runs", "postgresql-use-more-now", "latest");

verifyUseMoreNow();
verifyFunctionScan();
verifySafeOps();

console.log("verified PostgreSQL use-more-now scan and safe-ops receipts");

function verifyUseMoreNow() {
  const path = join(receiptRoot, "use-more-now-receipt.yaml");
  check(existsSync(path), "missing PostgreSQL use-more-now receipt");
  const receipt = readYaml(path);
  check(receipt.kind === "UseMoreNowReceipt", "PostgreSQL use-more-now receipt kind mismatch");
...
function verifyFunctionScan() {
  ...
  check(receipt.spec?.context?.space === "helm-postgresql-use-more-now", "PostgreSQL function scan receipt must target proof space");
...
function verifySafeOps() {
  ...
  check(receipt.spec?.context?.space === "helm-postgresql-use-more-now", "PostgreSQL safe-ops receipt must target proof space");
