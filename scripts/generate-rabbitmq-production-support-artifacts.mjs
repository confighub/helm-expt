...
if (mode === "--generate") {
  ...
  console.log(`wrote RabbitMQ support artifacts -> ${relativeRepo(outputRoot)}/`);
} else if (mode === "--verify") {
  ...
  console.log("verified RabbitMQ support artifacts");
} else {
  console.log(`Usage:
  node scripts/generate-rabbitmq-production-support-artifacts.mjs --generate
  node scripts/generate-rabbitmq-production-support-artifacts.mjs --verify`);
}
  ...
  return {
    ...
    spec: {
      ...
      productionSupportUse: {
        ...
        claim: "The mutable rendered image references for this RabbitMQ base were resolved to registry manifest digests at the recorded time.",
...
function buildImagePolicyDecision() {
  ...
  return {
    ...
    spec: {
      ...
      acceptedException: {
        reason:
          "The public proof target uses the upstream Bitnami legacy RabbitMQ image tag. The receipt set records the registry digest that tag resolved to at decision time.",
        operatorAction:
      ...
      evidence: [
        { path: relativeRepo(imageWorkdownPath), claim: "Summarizes RabbitMQ rendered image references." },
        { path: relativeRepo(imageReviewPath), claim: "Lists every rendered RabbitMQ image reference found in the object sets." },
        ...receipts.map((receipt) => ({
          path: receipt.path,
...
function buildSecurityDecision() {
  ...
  return {
    ...
    spec: {
      ...
      evidence: [
        { path: relativeRepo(scanWorkdownPath), claim: "Routes RabbitMQ scan findings to accept-or-patch-pdb-policy." },
        { path: relativeRepo(externalScanPath), claim: "Records kube-linter warning counts for RabbitMQ rendered object sets." },
        ...
        })),
        {
          ...
          claim: "Earlier production disposition accepts RabbitMQ scan warnings as production-review inputs.",
        },
      ],
      remainingSupportBlockers: [
  ...
  return {
    ...
    spec: {
      ...
      lifecycleModel: {
        ...
        storagePolicy:
          "The public proof uses the chart's default standalone StatefulSet PVC posture. StorageClass, backup, restore, queue recovery, and rollback remain target decisions.",
        excludedTopology:
          "existing-secret is a useful target-fact path but remains outside this production-support claim until its target password, Erlang cookie, custody, and rotation policy are separately reviewed.",
      },
...
function buildFreshEvidenceReceipt() {
  ...
  return {
    ...
    spec: {
      ...
      checks: [
        { name: "regular-helm-runtime", result: live.spec.legs.regularHelm.runtime.result, detail: "regular Helm RabbitMQ runtime passed" },
        { name: "confighub-apply-runtime", result: live.spec.legs.configHubKubectlApply.runtime.result, detail: "ConfigHub rendered objects applied by kubectl reached RabbitMQ readiness" },
...
function verifyImagePolicyDecision() {
  ...
  check(decision.spec.decision === "mutable-image-exception-accepted-for-target-scope", "RabbitMQ image policy decision mismatch");
  check(decision.spec.supportedBaseCandidate === supportedBase, "RabbitMQ image policy supported base mismatch");
}

function verifySecurityDecision() {
  const decision = decisionFile("security-decision.yaml", "ProductionSecurityDecision");
  check(decision.spec.decision === "pdb-policy-accepted-for-target-scope", "RabbitMQ security decision mismatch");
  check(decision.spec.findingSummary.totalFindings === Number(scanWorkdownRow().findingCount), "RabbitMQ security finding count mismatch");
  for (const row of externalScanRows()) {
    check(decision.spec.findingSummary.variants[row.variant].renderedObjectSetSHA256 === row.renderedSHA256, `RabbitMQ security rendered sha mismatch for ${row.variant}`);
  }
}

function verifyLifecycleDecision() {
  const decision = decisionFile("lifecycle-decision.yaml", "ProductionLifecycleDecision");
  check(decision.spec.decision === "lifecycle-observed-for-proof-scope", "RabbitMQ lifecycle decision mismatch");
  check(decision.spec.lifecycleModel.hookPolicy === "no-chart-hooks", "RabbitMQ lifecycle hook policy mismatch");
  check(decision.spec.observedLifecycleSignals.confighubOciArgo.argoSync === "Synced", "RabbitMQ lifecycle Argo sync mismatch");
  check(decision.spec.observedLifecycleSignals.confighubOciArgo.argoHealth === "Healthy", "RabbitMQ lifecycle Argo health mismatch");
}
...
function verifyFreshEvidenceReceipt() {
  ...
  const live = liveReceipt();
  check(receipt.spec.supportClaim.state === "fresh-target-evidence-passed", "RabbitMQ fresh evidence claim mismatch");
  check(receipt.spec.observedAt === live.spec.observedAt, "RabbitMQ fresh evidence observedAt mismatch");
  check(receipt.spec.oci.revision === live.spec.legs.configHubOciArgo.ociRevision, "RabbitMQ fresh evidence OCI revision mismatch");
  check(receipt.spec.oci.sync === "Synced", "RabbitMQ Argo sync mismatch");
  check(receipt.spec.oci.health === "Healthy", "RabbitMQ Argo health mismatch");
}
...
function imageReceipt(variant) {
  ...
  check(existsSync(path), `missing ${relativeRepo(path)}; run npm run rabbitmq:production-support`);
...
function imageWorkdownRow() {
  ...
  check(rows.length === 1, "expected one RabbitMQ image workdown row");
  return rows[0];
}
...
function externalScanRows() {
  ...
  check(rows.length === variants.length, "expected RabbitMQ external scan rows");
  return rows;
}
...
function scanWorkdownRow() {
  ...
  check(rows.length === 1, "expected one RabbitMQ scan workdown row");
  return rows[0];
}
