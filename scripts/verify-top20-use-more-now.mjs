...
const selected = chartsArg
  ...
  : TOP20_USE_MORE_NOW_CHARTS;

for (const chart of selected) verifyChart(chart);

console.log(`verified ${selected.length} top-20 use-more-now receipt set(s)`);
...
function verifyChart(chart) {
  const root = join(repoRoot, "runs", `${chart.slug}-use-more-now`, "latest");
  const demoRoot = join(repoRoot, "docs", "demo", chart.slug);
  const useMoreNowPath = join(root, "use-more-now-receipt.yaml");
  const functionScanPath = join(root, "function-scan-receipt.yaml");
  const safeOpsPath = join(root, "safe-ops-receipt.yaml");

  check(existsSync(useMoreNowPath), `${chart.slug} missing use-more-now receipt`);
  ...
  check(existsSync(join(demoRoot, "use-more-now.md")), `${chart.slug} missing use-more-now doc`);
  check(existsSync(join(demoRoot, "use-more-now-transcript.md")), `${chart.slug} missing use-more-now transcript`);

  const receipt = readYaml(useMoreNowPath);
  const functionScan = readYaml(functionScanPath);
  const safeOps = readYaml(safeOpsPath);

  check(receipt.kind === "UseMoreNowReceipt", `${chart.slug} use-more-now kind mismatch`);
