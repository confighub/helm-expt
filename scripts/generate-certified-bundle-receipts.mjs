...
function buildSpaceGuide({ name, producer, sourceLine, contentsKind, files, verdict, routeFiles, uploadCommand, manualNotes }) {
  const lines = [];
  lines.push(`# ${name}`);
  lines.push("");
  lines.push("<!-- Generated by npm run certified-bundles. Do not edit by hand. -->");
  lines.push("");
  lines.push(manualNotes ? `This configuration bundle can be ingested into a Space with cub v0.5.5. The server splits Kubernetes resources into Units. ${sourceLine}` : `This Space holds one certified bundle, ingested one Unit per file. ${sourceLine}`);
  lines.push("");
  ...
  if (routeFiles.length > 0) {
    lines.push(manualNotes ? "This bundle carries route declarations and retained observations. They are not executable controllers: complete and verify these prerequisites manually before delivery." : "This bundle ships the following routes. A delivery runtime executes them; they are not documentation.");
    lines.push("");
