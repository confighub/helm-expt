...
function buildCatalogBundleReceipt({ recipe, packageRoot, base, chartName, verdictFile, notes, hookObservation, secretsCarryConfiguration }) {
  ...
  if (hookObservation) {
    ...
    if (hookObservation.webhookCA) {
      const caRel = `data/certified-bundles/routes/catalog/${slug}/webhook-ca.yaml`;
      const caRoute = structuredClone(route);
      caRoute.metadata.name = `${slug}-webhook-ca`;
      caRoute.spec.quirkClass = "webhook-ca";
      caRoute.spec.discharges = "The target-owned admission certificate must exist and its CA must be patched into all webhook configurations before admission is ready. The retained manual lifecycle observation records creation, patching and readiness checks.";
      const caText = `${toYaml(caRoute)}\n`;
      emittedRoutes.push({ path: repoPath(caRel), contents: caText });
      routeFiles.push({ path: caRel, sha256: sha256(caText), bytes: Buffer.byteLength(caText), role: "route: webhook-ca" });
    }
  }
  ...
  if (hookObservation) {
    ...
    if (hookObservation.webhookCA) {
      const ca = dispositions.find((entry) => entry.class === "webhook-ca");
      check(ca.finding === "present", "a webhook companion requires a webhook in the payload");
      ca.disposition = `manual certificate creation, patching and readiness checks declared by the route this bundle ships at ${routeRefs["webhook-ca"]}`;
      ca.companionRequired = "lifecycle-job";
      ca.evidence = hookObservation.observationRel;
    }
  }
...
const EKS_INFERENCE_COMPONENTS = [
  {
    ...
    plane: "hub",
    order: 0,
    description: "Shared values used by the other components; stored in ConfigHub and never applied to a cluster.",
  ...
  },
  {
    ...
    plane: "mgmt",
    order: 1,
    description: "VPC, subnets, NAT, route tables, and security group.",
    sourceKind: "literal-yaml",
    contentsKind: "literal-config",
  ...
  },
  {
    ...
    plane: "mgmt",
    order: 2,
    description: "Cluster and node IAM roles, EKS control plane, addons, and system node group.",
    sourceKind: "literal-yaml",
    contentsKind: "literal-config",
  ...
  },
  {
    ...
    plane: "workload",
    order: 1,
    description: "NVIDIA device plugin, so GPU nodes advertise their hardware.",
    sourceKind: "helm-chart",
    contentsKind: "rendered-config",
  ...
  },
  {
    ...
    plane: "workload",
    order: 2,
    description: "CPU and GPU checks, a chat client, and the sample vLLM deployment.",
    sourceKind: "literal-yaml",
    contentsKind: "literal-config",
...
const CATALOG_BUNDLES = [
  ...
  { repo: "bitnami", chart: "apache", version: "11.4.29", base: "default" },
  { repo: "bitnami", chart: "elasticsearch", version: "22.1.6", base: "default" },
  { repo: "bitnami", chart: "memcached", version: "8.5.5", base: "default" },
  { repo: "bitnami", chart: "opensearch", version: "2.0.10", base: "default" },
  { repo: "bitnami", chart: "phpmyadmin", version: "20.0.0", base: "default" },
  { repo: "bitnami", chart: "spark", version: "10.0.3", base: "default" },
  { repo: "bitnami", chart: "zookeeper", version: "13.8.7", base: "default" },
...
function buildSpaceGuide({ name, producer, sourceLine, contentsKind, files, verdict, routeFiles, uploadCommand, manualNotes, guideNotes, currentServerUnits = false }) {
  const lines = [];
  lines.push(`# ${name}`);
  lines.push("");
  ...
  lines.push("");
  if (manualNotes) {
    lines.push(`This configuration bundle can be ingested into a Space with cub v0.5.5. The server splits Kubernetes resources into Units. ${sourceLine}`);
  } else {
  lines.push(
    currentServerUnits
      ? `This Space holds one certified bundle. The current server creates resource Units from its contents. ${sourceLine}`
      : `This Space holds one certified bundle, ingested one Unit per file. ${sourceLine}`,
  );
  }
  lines.push("");
  ...
  if (routeFiles.length > 0) {
    if (manualNotes) {
      lines.push("This bundle carries route declarations and retained observations. They are not executable controllers: complete and verify these prerequisites manually before delivery.");
    } else {
    lines.push(
      guideNotes
  ...
  lines.push(manualNotes ? "## How to ingest it" : "## How it was ingested");
  lines.push("");
  lines.push("```sh");
  lines.push(manualNotes ? uploadCommand.replace(" --granularity per-file", "") : uploadCommand);
  lines.push("```");
  lines.push("");
  lines.push(
    manualNotes ? "After publication, replace `<bundle>` with the published OCI reference. Ingestion records its resolved digest as a `confighub.com/external-source` annotation. Ingestion does not deploy the configuration or execute its manual routes." : "The resolved digest is recorded on this Space as a `confighub.com/external-source` annotation, so the exact bytes installed here stay auditable.",
  );
  lines.push("");
  return manualNotes ? `${lines.join("\n").trimEnd()}\n` : `${lines.join("\n")}\n`;
}
...
function eksInferenceStackGuide(receipts) {
  ...
  const entries = EKS_INFERENCE_COMPONENTS.map((component) => {
    ...
    check(receipt, `missing eks-inference receipt for ${component.name}`);
    return { component, receipt };
  });
  const commits = new Set(entries.map(({ receipt }) => receipt.value.spec.producer.commit));
  check(commits.size === 1, "eks-inference component receipts do not name one producer commit");
  const [commit] = commits;
  const observed = new Set(
    entries.map(({ component }) =>
      readYaml(repoPath(eksInferenceWitnessPath(component.name))).spec.observedAt,
    ),
  );
  check(observed.size === 1, "eks-inference component witnesses were not observed together");
  const [observedAt] = observed;
  const planeName = {
    hub: "ConfigHub only",
    mgmt: "Management cluster",
    workload: "EKS workload cluster",
  };
  const relativeLink = (path) => `./${path.replace(/^data\/certified-bundles\//, "")}`;
  const lines = [];
  lines.push("# EKS inference: eight checked configuration bundles");
  lines.push("");
  lines.push("<!-- Generated by npm run certified-bundles. Do not edit by hand. -->");
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    `The packages below were read from GHCR on ${observedAt}. Every extracted file matched [the producer's committed configuration](https://github.com/confighub/eks-inference/blob/${commit}/components.yaml) at commit [${commit.slice(0, 12)}](https://github.com/confighub/eks-inference/commit/${commit}).`,
  );
  lines.push("");
  lines.push("## What runs where");
  lines.push("");
  lines.push("| Plane | Order | Component | Purpose | Source | Extra handling |");
  lines.push("| --- | ---: | --- | --- | --- | --- |");
  for (const { component, receipt } of entries) {
    const spec = receipt.value.spec;
    const routes = spec.bundle.files.filter((file) => String(file.role ?? "").startsWith("route:"));
    const routeText = routes.length > 0
      ? routes.map((route) => `[${route.role.replace("route:", "").trim()}](${relativeLink(route.path)})`).join(", ")
      : "None recorded";
    lines.push(
      `| ${planeName[component.plane]} | ${component.order} | [${component.name}](${relativeLink(receipt.rel)}) | ${component.description} | ${component.sourceKind === "helm-chart" ? "Helm render" : "Literal YAML"} | ${routeText} |`,
    );
  }
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push("## Inspect the configuration first");
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push("```sh");
  lines.push(`oras pull ${entries.at(-1).receipt.value.spec.bundle.reference.replace(/:latest$/, `@${entries.at(-1).receipt.value.spec.bundle.manifestDigest}`)}`);
  lines.push("```");
  lines.push("");
  lines.push("## Build the configuration in ConfigHub");
  lines.push("");
  lines.push("```sh");
  lines.push("cub plugin install confighub/eks-inference");
  lines.push("cub eksinf install");
  lines.push("cub eksinf sandbox up");
  lines.push("```");
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push("| Component | Exact OCI | Receipt and guide |");
  lines.push("| --- | --- | --- |");
  for (const { component, receipt } of entries) {
    const spec = receipt.value.spec;
    const exact = `${spec.bundle.reference.replace(/:latest$/, "")}@${spec.bundle.manifestDigest}`;
    const guide = receipt.rel
      .replace("data/certified-bundles/receipts/", "data/certified-bundles/guides/")
      .replace(/receipt\.yaml$/, "space-guide.md");
    lines.push(
      `| ${component.name} | \`${exact}\` | [receipt](${relativeLink(receipt.rel)}) · [Space guide](${relativeLink(guide)}) |`,
    );
  }
  lines.push("");
  lines.push("## What this evidence does not prove");
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    `[Follow the producer's current walkthrough](https://github.com/confighub/eks-inference/tree/${commit}) when you are ready for the real AWS and GPU path.`,
  );
  lines.push("");
  return lines.join("\n");
}
...
function buildAll() {
  const receipts = [
    {
      ...
      value: buildCatalogBundleReceipt({
        ...
        packageRoot: "runs/prometheus-operator-minimal-candidate/packages/prometheus-community/kube-prometheus-stack/87.19.2",
        base: "minimal",
        chartName: "prometheus-community/kube-prometheus-stack",
        verdictFile: "flattening-safety-verdict-minimal.yaml",
        hookObservation: {
          observationRel: "runs/prometheus-operator-minimal-live/receipt.yaml",
          hookDocCount: 7,
          webhookCA: true,
        },
        notes: "A separately published minimal configuration profile derived from the exact chart, not a replacement of its immutable three-base installer. Applications and their ServiceMonitors remain separate. CRD establishment and admission certificate creation/patching/cleanup are explicit manual prerequisites. The retained kind run observed this payload and a synthetic app scrape once; upgrades, rollback and GitOps execution remain unproven.",
      }),
    },
    ...
    ...CATALOG_BUNDLES.map((entry) => {
      ...
      return {
        ...
        currentServerUnits: entry.currentServerUnits,
      };
    }),
  ...
  for (const receipt of receipts) {
    ...
    const guide = buildSpaceGuide({
      ...
      routeFiles,
      manualNotes: receipt.manualNotes,
      guideNotes: receipt.guideNotes,
      currentServerUnits: receipt.currentServerUnits,
      uploadCommand:
