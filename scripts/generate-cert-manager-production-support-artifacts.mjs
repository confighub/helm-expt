...
const chart = "jetstack/cert-manager";
const chartSlug = "jetstack-cert-manager";
const version = "v1.20.2";
const supportedBase = "crds-enabled";
const variants = ["default", "crds-enabled"];
...
const defaultLifecyclePath = join(repoRoot, "runs", "lifecycle-observations", "cert-manager-eso", `${chartSlug}-default`, "receipt.yaml");
...
if (mode === "--generate") {
  ...
  console.log(`wrote cert-manager support artifacts -> ${relativeRepo(outputRoot)}/`);
} else if (mode === "--verify") {
  ...
  console.log("verified cert-manager support artifacts");
} else {
  console.log(`Usage:
  node scripts/generate-cert-manager-production-support-artifacts.mjs --generate
  node scripts/generate-cert-manager-production-support-artifacts.mjs --verify`);
}
  ...
  return {
    ...
    spec: {
      ...
      productionSupportUse: {
        ...
        claim: "The mutable rendered image references for this cert-manager base were resolved to registry manifest digests at the recorded time.",
...
function buildImagePolicyDecision() {
  ...
  return {
    ...
    spec: {
      ...
      claim:
        "The rendered cert-manager bases still contain mutable image tags, but each rendered image reference has digest-resolution evidence. For this public controller support scope, the mutable tag is accepted as a target-scoped exception while stricter environments remain free to require digest-pinned bases or image overrides.",
      ...
      acceptedException: {
        reason:
          "The public proof target uses upstream cert-manager controller image tags. The receipt set records the registry digest each tag resolved to at decision time, so reviewers can rerun the decision when upstream tags move.",
        operatorAction:
      ...
      limits: [
        ...
        "This does not make cert-manager production-supported by itself.",
        "If an upstream image tag is retargeted, rerun digest resolution and re-review this decision before promotion.",
      ],
      evidence: [
        { path: relativeRepo(imageWorkdownPath), claim: "Summarizes cert-manager rendered image references." },
        { path: relativeRepo(imageReviewPath), claim: "Lists every rendered image reference found in the cert-manager object sets." },
        ...receipts.map((receipt) => ({
          path: receipt.path,
      ...
      remainingSupportBlockers: [
        "Record security/resource-policy acceptance and lifecycle/fresh target-scoped evidence for the declared support scope.",
      ],
    },
  };
}
...
function buildSecurityDecision() {
  ...
  return {
    ...
    spec: {
      ...
      evidence: [
        { path: relativeRepo(scanWorkdownPath), claim: "Routes cert-manager scan findings to add-resource-policy." },
        { path: relativeRepo(externalScanPath), claim: "Records kube-linter warning counts for default and crds-enabled rendered object sets." },
...
function buildLifecycleDecision() {
  const lifecycle = lifecycleReceipt(lifecyclePath, supportedBase);
  const defaultLifecycle = lifecycleReceipt(defaultLifecyclePath, "default");
  ...
  return {
    ...
    spec: {
      ...
      evidence: [
        { path: relativeRepo(lifecyclePath), claim: "Lifecycle observation for the crds-enabled base." },
        { path: relativeRepo(defaultLifecyclePath), claim: "Lifecycle observation for the default target-prerequisite posture." },
        { path: relativeRepo(liveReceiptPath), claim: "ConfigHub OCI/Argo live parity for the crds-enabled base." },
        {
        ...
        {
          path: `data/production-disposition/receipts/${chartSlug}/webhook-readiness-and-failure-policy.yaml`,
  ...
  return {
    ...
    spec: {
      ...
      supportClaim: {
        ...
        detail: "Fresh target-scoped ConfigHub OCI and Argo evidence passed for the declared cert-manager crds-enabled support scope.",
      },
      limits: [
        "This supports the recorded cub-lk vanilla kind Argo OCI scope, not every Kubernetes cluster.",
        "Issuer, ClusterIssuer, ACME/provider configuration, and certificate workloads are outside this base support claim.",
...
function verifyImagePolicyDecision() {
  ...
  check(decision.spec.decision === "mutable-image-exception-accepted-for-target-scope", "cert-manager image policy decision mismatch");
  check((decision.spec.variantsCovered ?? []).length === variants.length, "cert-manager image policy variants mismatch");
}

function verifySecurityDecision() {
  const decision = decisionFile("security-decision.yaml", "ProductionSecurityDecision");
  check(decision.spec.decision === "resource-policy-accepted-for-target-scope", "cert-manager security decision mismatch");
  check(decision.spec.findingSummary.totalFindings === Number(scanWorkdownRow().findingCount), "cert-manager security finding count mismatch");
  for (const row of externalScanRows()) {
    check(decision.spec.findingSummary.variants[row.variant].renderedObjectSetSHA256 === row.renderedSHA256, `cert-manager security rendered sha mismatch for ${row.variant}`);
  }
}

function verifyLifecycleDecision() {
  const decision = decisionFile("lifecycle-decision.yaml", "ProductionLifecycleDecision");
  check(decision.spec.decision === "lifecycle-observed-for-proof-scope", "cert-manager lifecycle decision mismatch");
  check(decision.spec.supportedBaseCandidate === supportedBase, "cert-manager lifecycle supported base mismatch");
  check(decision.spec.observedLifecycleSignals.supportedBase.checks["server-dry-run-api-object"] === "pass", "cert-manager lifecycle server dry-run missing");
  check(decision.spec.observedLifecycleSignals.confighubOciArgo.argoSync === "Synced", "cert-manager lifecycle Argo sync mismatch");
}
...
function verifyFreshEvidenceReceipt() {
  ...
  const live = liveReceipt();
  check(receipt.spec.supportClaim.state === "fresh-target-evidence-passed", "cert-manager fresh evidence claim mismatch");
  check(receipt.spec.observedAt === live.spec.observedAt, "cert-manager fresh evidence observedAt mismatch");
  check(receipt.spec.oci.revision === live.spec.legs.configHubOciArgo.ociRevision, "cert-manager fresh evidence OCI revision mismatch");
  check(receipt.spec.oci.sync === "Synced", "cert-manager Argo sync mismatch");
  check(receipt.spec.oci.health === "Healthy", "cert-manager Argo health mismatch");
}
...
function imageReceipt(variant) {
  ...
  check(existsSync(path), `missing ${relativeRepo(path)}; run npm run cert-manager:production-support`);
...
function imageWorkdownRow() {
  ...
  check(rows.length === 1, "expected one cert-manager image workdown row");
  return rows[0];
}
...
function externalScanRows() {
  ...
  check(rows.length === variants.length, "expected cert-manager external scan rows");
  return rows;
}
...
function scanWorkdownRow() {
  ...
  check(rows.length === 1, "expected one cert-manager scan workdown row");
  return rows[0];
}

function lifecycleReceipt(path, base) {
  check(existsSync(path), `missing ${relativeRepo(path)}`);
  const receipt = readYaml(path);
  check(receipt.kind === "LifecycleObservationReceipt", `${relativeRepo(path)} kind mismatch`);
  check(receipt.spec?.chart === chart, `${relativeRepo(path)} chart mismatch`);
  check(receipt.spec?.version === version, `${relativeRepo(path)} version mismatch`);
