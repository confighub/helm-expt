...
// generation receipt without requiring Docker or downloaded release assets.
...
function staticFacts() {
  ...
  const sbomSignature = readStatement(sbomBundlePath);
  ...
  check(sbomSubject, "SBOM attestation has no SBOM subject");
  ...
  check(sbomSubject.sha256 === sha256(readFileSync(sbomPath)), "SBOM differs from its signed subject");
  check(
    sbomSubject.sha256 === generation.spec?.source?.sbom?.sha256,
    "SBOM attestation differs from the generation receipt",
  );
  for (const signature of [recipeSignature, binarySignature, sbomSignature]) {
    check(
...
function run() {
  ...
  const binaryVerified = cosignSubject(
    expectedIdentity,
    binaryPath,
...
function runDocker(args, mountedSubjectDir = "") {
  const dockerArgs = [
    "run", "--rm", "--network", "none",
    "-v", `${signatureRoot}:/sig:ro`,
  ];
  if (mountedSubjectDir) dockerArgs.push("-v", `${mountedSubjectDir}:/subject:ro`);
...
function buildReceipt(facts, observedAt) {
  return {
    ...
    spec: {
      ...
      sbom: {
        path: relativeRepo(sbomPath),
        sha256: facts.sbomSubject.sha256,
        bundle: relativeRepo(sbomBundlePath),
        bundleSha256: facts.sbomSignature.bundleSha256,
        attestedSha256: facts.sbomSubject.sha256,
      },
      signature: {
        ...
        sbom: {
          mediaType: facts.sbomSignature.mediaType,
          predicateType: facts.sbomSignature.predicateType,
          transparencyLog: facts.sbomSignature.transparencyLog,
          signedTimestampCount: facts.sbomSignature.signedTimestampCount,
        },
      },
      result: {
        recipeCatalogSignature: "pass",
        binaryAttestation: "pass",
        sbomAttestation: "pass",
      ...
      limits: [
        ...
        "The SBOM attestation covers the retained SBOM file, not the generated recipe or bundle.",
...
function verifyReceipt(receipt, facts) {
  ...
  check(receipt.spec?.sbom?.bundleSha256 === facts.sbomSignature.bundleSha256, "SBOM bundle drifted");
  check(receipt.spec?.sbom?.attestedSha256 === facts.sbomSubject.sha256, "SBOM subject drifted");
...
All three signatures identify the exact NVIDIA release workflow for \`${version}\`:
