...
function buildImagePolicyDecision() {
  ...
  return {
    ...
    spec: {
      ...
      claim:
        "The rendered MySQL bases still contain mutable image tags, but each rendered image reference has digest-resolution evidence. For the static-passwords public proof scope, the mutable tags are accepted as a target-scoped exception while stricter environments remain free to require digest-pinned bases or image overrides.",
...
function buildLifecycleDecision() {
  ...
  return {
    ...
    spec: {
      ...
      decidedAt: "2026-06-09",
      claim:
        "The MySQL static-passwords base has no Helm hook objects and reaches readiness through regular Helm, cub installer apply, and ConfigHub OCI/Argo. The generated root password is bound before render and the rendered Secret is separated by cub installer rather than hidden in workload units.",
      lifecycleModel: {
  ...
  return {
    ...
    spec: {
      ...
      supportClaim: {
        ...
        detail: "Fresh target-scoped ConfigHub OCI and Argo evidence passed for the declared MySQL static-passwords support scope.",
      },
      limits: [
