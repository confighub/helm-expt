...
const chart = "bitnami/mysql";
const chartSlug = "bitnami-mysql";
...
if (mode === "--generate") {
  ...
  console.log(`wrote MySQL support artifacts -> ${relativeRepo(outputRoot)}/`);
} else if (mode === "--verify") {
  ...
  console.log("verified MySQL support artifacts");
} else {
  console.log(`Usage:
  node scripts/generate-mysql-production-support-artifacts.mjs --generate
  node scripts/generate-mysql-production-support-artifacts.mjs --verify`);
}
  ...
  return {
    ...
    spec: {
      ...
      productionSupportUse: {
        ...
        claim: "The mutable rendered image references for this MySQL base were resolved to registry manifest digests at the recorded time.",
...
function buildImagePolicyDecision() {
  ...
  return {
    ...
    spec: {
      ...
      claim:
        "The rendered MySQL bases still contain mutable image tags, but each rendered image reference has digest-resolution evidence. For the generated-passwords public proof scope, the mutable tags are accepted as a target-scoped exception while stricter environments remain free to require digest-pinned bases or image overrides.",
      ...
      acceptedException: {
        reason:
          "The public proof target uses the upstream Bitnami legacy MySQL image tag. The receipt set records the registry digest that tag resolved to at decision time.",
        operatorAction:
      ...
      evidence: [
        { path: relativeRepo(imageWorkdownPath), claim: "Summarizes MySQL rendered image references." },
        { path: relativeRepo(imageReviewPath), claim: "Lists every rendered MySQL image reference found in the object sets." },
        ...receipts.map((receipt) => ({
          path: receipt.path,
  ...
  return {
    ...
    spec: {
      ...
      limits: [
        ...
        "This does not support the existing-secret base for production use.",
        "Production storage, backup, restore, failover, credential rotation, and SLO policy remain separate decisions.",
      ],
      evidence: [
        { path: relativeRepo(scanWorkdownPath), claim: "Routes MySQL scan findings to accept-or-patch-pdb-policy." },
        { path: relativeRepo(externalScanPath), claim: "Records kube-linter warning counts for MySQL rendered object sets." },
        ...
        })),
        {
          ...
          claim: "Earlier production disposition accepts MySQL scan warnings as production-review inputs.",
        },
      ],
      remainingSupportBlockers: [
...
function buildLifecycleDecision() {
  ...
  return {
    ...
    spec: {
      ...
      decidedAt: "2026-06-09",
      claim:
        "The MySQL generated-passwords base has no Helm hook objects and reaches readiness through regular Helm, cub installer apply, and ConfigHub OCI/Argo. The generated root password is bound before render and the rendered Secret is separated by cub installer rather than hidden in workload units.",
      lifecycleModel: {
        hookPolicy: "no-chart-hooks",
        selectedTopology: "primary-statefulset-generated-passwords",
        generatedFacts:
        ...
        storagePolicy:
          "The public proof uses the chart's default primary StatefulSet PVC posture. StorageClass, backup, restore, and rollback remain target decisions.",
        excludedTopology:
          "existing-secret is a useful target-fact path but remains outside this production-support claim until its target credential and rotation policy are separately reviewed.",
      },
      ...
      limits: [
        "This supports the generated-passwords public proof base, not every MySQL deployment topology.",
...
function buildFreshEvidenceReceipt() {
  ...
  return {
    ...
    spec: {
      ...
      checks: [
        { name: "regular-helm-runtime", result: live.spec.legs.regularHelm.runtime.result, detail: "regular Helm MySQL runtime passed" },
        { name: "confighub-apply-runtime", result: live.spec.legs.configHubKubectlApply.runtime.result, detail: "ConfigHub rendered objects applied by kubectl reached MySQL readiness" },
      ...
      supportClaim: {
        ...
        detail: "Fresh target-scoped ConfigHub OCI and Argo evidence passed for the declared MySQL generated-passwords support scope.",
      },
      limits: [
...
function verifyImagePolicyDecision() {
  ...
  check(decision.spec.decision === "mutable-image-exception-accepted-for-target-scope", "MySQL image policy decision mismatch");
  check(decision.spec.supportedBaseCandidate === supportedBase, "MySQL image policy supported base mismatch");
}

function verifySecurityDecision() {
  const decision = decisionFile("security-decision.yaml", "ProductionSecurityDecision");
  check(decision.spec.decision === "pdb-policy-accepted-for-target-scope", "MySQL security decision mismatch");
  check(decision.spec.findingSummary.totalFindings === Number(scanWorkdownRow().findingCount), "MySQL security finding count mismatch");
  for (const row of externalScanRows()) {
    check(decision.spec.findingSummary.variants[row.variant].renderedObjectSetSHA256 === row.renderedSHA256, `MySQL security rendered sha mismatch for ${row.variant}`);
  }
}

function verifyLifecycleDecision() {
  const decision = decisionFile("lifecycle-decision.yaml", "ProductionLifecycleDecision");
  check(decision.spec.decision === "lifecycle-observed-for-proof-scope", "MySQL lifecycle decision mismatch");
  check(decision.spec.lifecycleModel.hookPolicy === "no-chart-hooks", "MySQL lifecycle hook policy mismatch");
  check(decision.spec.observedLifecycleSignals.confighubOciArgo.argoSync === "Synced", "MySQL lifecycle Argo sync mismatch");
  check(decision.spec.observedLifecycleSignals.confighubOciArgo.argoHealth === "Healthy", "MySQL lifecycle Argo health mismatch");
}
...
function verifyFreshEvidenceReceipt() {
  ...
  const live = liveReceipt();
  check(receipt.spec.supportClaim.state === "fresh-target-evidence-passed", "MySQL fresh evidence claim mismatch");
  check(receipt.spec.observedAt === live.spec.observedAt, "MySQL fresh evidence observedAt mismatch");
  check(receipt.spec.oci.revision === live.spec.legs.configHubOciArgo.ociRevision, "MySQL fresh evidence OCI revision mismatch");
  check(receipt.spec.oci.sync === "Synced", "MySQL Argo sync mismatch");
  check(receipt.spec.oci.health === "Healthy", "MySQL Argo health mismatch");
}
...
function imageReceipt(variant) {
  ...
  check(existsSync(path), `missing ${relativeRepo(path)}; run npm run mysql:production-support`);
...
function imageWorkdownRow() {
  ...
  check(rows.length === 1, "expected one MySQL image workdown row");
  return rows[0];
}
...
function externalScanRows() {
  ...
  check(rows.length === variants.length, "expected MySQL external scan rows");
  return rows;
}
...
function scanWorkdownRow() {
  ...
  check(rows.length === 1, "expected one MySQL scan workdown row");
  return rows[0];
}
