...
const summaryPath = join(repoRoot, "data", "helm-org", "summary.md");
const previousSummary = existsSync(summaryPath) ? readFileSync(summaryPath, "utf8") : "";
const detailOffset = previousSummary.indexOf("\n## ");
const retainedDetails = detailOffset === -1 ? "" : previousSummary.slice(detailOffset);
write(
  summaryPath,
  `# helm-catalog Org, Wave 1\n\nGenerated by scripts/sync-helm-org.mjs --sync against org '${orgArg}'.\n\n- planned: ${plan.length} base Space(s)\n- created: ${created}\n- already present: ${skipped}\n- units in org (planned spaces): ${unitsTotal}\n- stopped early: ${stopped || "no"}\n\nOne Space per (chart, version, base variant) base variant, immutable; labels carry Component, Variant, ChartVersion, HookRoute, CrdRoute, SecretRoute, RouteDisposition from committed catalog data. Rows in [wave1.csv](./wave1.csv).${retainedDetails || "\n"}`,
);
