...
// matrix (cub-scout vs ArgoCD vs Flux) + the example rollup.
...
function summaryMd(matrix, examples) {
  ...
  o.push("Day-1 **dry-run** and day-2 **drift** are the same operation — *desired vs live* — so they are one engine. cub-scout already does the boolean form (`object-set-matches`, `drift`); this is the field-level form. It diffs the **held render-once desired data** (deterministic) against the live cluster via **server-side dry-run** (the accurate technique Argo and Flux use), is **GitOps-tool-agnostic** (works under Argo, Flux, or cub-direct), and adds value-provenance, fleet blast-radius, authority, and honest residue. See [the design](../../docs/user/cub-scout-diff-design.md).", "");

  o.push("## Better than ArgoCD and Flux", "");
...
function html(matrix, examples) {
  ...
  p.push("<p class=\"note\">One differ (desired vs live) that serves day-1 dry-run and day-2 drift, works under Argo / Flux / cub-direct, and adds the layers per-tool diffs lack. The <span style=\"background:#d6f5d6\">&nbsp;cub-scout&nbsp;</span> column is the advantage.</p>");
