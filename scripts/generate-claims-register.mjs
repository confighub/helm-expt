...
const claims = [
  ...
  },
  {
    ...
    verify: "npm run hooks:lifecycle:verify; npm run lifecycle:boundary:verify; npm run lifecycle:gatekeeper-hooks:verify; npm run kps:lifecycle-route:verify; npm run hooks-crds-app:verify",
    limit: "The Kube Prometheus Stack fresh-install sequence passed through the direct script. Its Argo CD, Flux, and upgrade paths remain unproved, and other charts still need their own route decisions and receipts.",
  },
  {
    ...
    verify: "npm run lifecycle:cert-manager-eso:verify; npm run lifecycle:boundary:verify; npm run kps:lifecycle-route:verify",
    limit: "The direct Kube Prometheus Stack install proves CRD ordering, webhook certificate setup, and readiness checks for one version on kind. CRD upgrades and controller-specific delivery remain per-chart decisions.",
  },
  {
