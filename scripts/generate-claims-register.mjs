...
const claims = [
  ...
  },
  {
    ...
    verify: "npm run hooks:lifecycle:verify; npm run lifecycle:boundary:verify; npm run lifecycle:gatekeeper-hooks:verify; npm run kps:lifecycle-route:verify; npm run kps:gitops-lifecycle:verify; npm run kps:confighub-lifecycle-promotion:verify; npm run hooks-crds-app:verify",
    limit: "The Kube Prometheus Stack no-crds sequence passed a fresh install and the 85.3.3 to 86.1.0 upgrade through Argo CD and Flux. A separate ConfigHub run retained, approved, promoted, published, and delivered the same upgrade through Argo CD. A person or automation still starts the recorded Job-replacement step. Rollback, long soak, automatic route selection, and other charts need separate proof.",
  },
  {
    ...
    verify: "npm run lifecycle:cert-manager-eso:verify; npm run lifecycle:boundary:verify; npm run kps:lifecycle-route:verify; npm run kps:gitops-lifecycle:verify; npm run kps:confighub-lifecycle-promotion:verify",
    limit: "The Kube Prometheus Stack proof covers namespace preservation, CRD ordering, webhook certificate setup, operator-created workloads, and the 85.3.3 to 86.1.0 no-crds upgrade through Argo CD and Flux on kind. Rollback, longer soak, broader migration behavior, and other charts remain separate decisions.",
  },
  {
  ...
  },
  {
    ...
    area: "delivery",
    status: "partial",
    claim: "Selected configurations have live receipts for portable OCI delivery through Argo CD or Flux, with route work and workload health recorded separately.",
    evidence: [
