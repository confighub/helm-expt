...
const proofRoot = join(repoRoot, "recipes", "grafana", "grafana", "10.5.15");
const packageRoot = join(repoRoot, "packages", "grafana", "grafana", "10.5.15");
...
if (mode === "--generate-proof") {
  ...
  console.log(`Usage:
  node scripts/grafana-proof.mjs --generate-proof
  node scripts/grafana-proof.mjs --generate-package
  node scripts/grafana-proof.mjs --verify-proof
  node scripts/grafana-proof.mjs --verify-proof-self-test
  node scripts/grafana-proof.mjs --verify-package
  node scripts/grafana-proof.mjs --compare`);
}
...
function generateProof() {
  ...
  writeYaml(join(proofRoot, "source-lock.yaml"), {
    ...
    kind: "SourceLock",
    metadata: { name: "grafana-grafana-10.5.15" },
    spec: {
      ...
      evidence: {
        harnessReceipt: "../../../../data/adversarial10/charts/grafana-grafana-10.5.15/render-receipt.yaml",
      },
    },
  });
    ...
    spec: {
      points: [
        ...
        },
        {
          ...
          note: "The generated-passwords variant binds the generated Grafana admin password before render so Helm output is deterministic.",
        },
        {
          category: "target-facts",
          status: "variant-controlled",
          evidence: "admin.existingSecret",
          note: "The existing-secret-ingress variant declares the target Secret instead of rendering one.",
        },
  ...
  writeYaml(join(proofRoot, "recipe.yaml"), {
    apiVersion: "helm-expt.confighub.com/v1alpha1",
    kind: "Recipe",
    metadata: { name: "grafana-grafana", version: chart.version },
    spec: {
      ...
      currentExecutableFixture: {
        installerPackage: "../../../../packages/grafana/grafana/10.5.15",
        setupCommand: [
          "cub",
          "installer",
          "setup",
  ...
  for (const variant of variants) {
    ...
    const inventory = {
      ...
      metadata: { name: `grafana-grafana-${chart.version}-${variant.name}-r001` },
      spec: {
    ...
    writeYaml(join(receiptsRoot, "render-receipt.yaml"), {
      apiVersion: "helm-expt.confighub.com/v1alpha1",
      kind: "RenderReceipt",
      metadata: { name: `grafana-${variant.name}-r001` },
      spec: {
    ...
    writeYaml(join(receiptsRoot, "helm-equivalence-receipt.yaml"), {
      ...
      spec: {
        ...
        classifications: [
          { identity: "v1|Namespace||grafana", classification: "installer-support-object", disposition: "allowed" },
        ],
        result: "pass",
        evidenceCommand: "npm run grafana:compare",
      },
    });
  ...
  writeYaml(join(packageRoot, "installer.yaml"), {
    ...
    spec: {
      bases: variants.map((variant, index) => ({
        ...
        description: `grafana ${variant.displayName} variant rendered from grafana/grafana@${chart.version}`,
      })),
    },
  });
  write(
    join(packageRoot, "README.md"),
    `# grafana/grafana ${chart.version} Installer Package

This package is generated from the grafana proof artifacts.
...
`,
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), "grafana-installer-package-"));
  try {
    const firstPackage = join(tempRoot, "grafana-10.5.15-a.tgz");
    const secondPackage = join(tempRoot, "grafana-10.5.15-b.tgz");
    ...
    writeYaml(receiptPath, {
      ...
      spec: {
        ...
        deterministicBundle: {
          command: `cub installer package ${packageRelative} -o <tmp>/grafana-10.5.15.tgz`,
          sha256: firstSHA,
        ...
        setupChecks: variants.map((variant) => ({
          ...
          base: variant.base,
          command: `cub installer setup --pull ${packageRelative} --base ${variant.base} --work-dir <tmp> --non-interactive --namespace grafana`,
          ...
          allowedCubOnlyObjects: ["v1|Namespace||grafana"],
        })),
      },
    });
  } finally {
...
function verifyProof(root = proofRoot) {
  const requiredFiles = [
    ...
    "effective-values-existing-secret-ingress.yaml",
    "recipe.yaml",
    "variants/generated-passwords/variant.yaml",
    "variants/existing-secret-ingress/variant.yaml",
    ...
    "revisions/existing-secret-ingress/r001/variant-revision.yaml",
    "revisions/existing-secret-ingress/r001/rendered/release-objects.yaml",
    "revisions/existing-secret-ingress/r001/rendered/object-inventory.yaml",
    "revisions/existing-secret-ingress/r001/receipts/helm-equivalence-receipt.yaml",
    "revisions/existing-secret-ingress/r001/receipts/render-receipt.yaml",
    "revisions/existing-secret-ingress/r001/receipts/scan-receipt.yaml",
    "revisions/existing-secret-ingress/r001/receipts/install-gate.yaml",
  ];
  ...
  check(sourceLock.spec.chart === "grafana", "source chart mismatch");
  check(sourceLock.spec.version === "10.5.15", "source version mismatch");
  ...
  console.log("verified grafana proof artifacts");
}

function verifyProofSelfTest() {
  const tempRoot = mkdtempSync(join(tmpdir(), "grafana-proof-self-test-"));
  try {
    ...
    console.log("self-test passed: grafana rendered object tampering is rejected");
  } finally {
    rmSync(tempRoot, { recursive: true, force: true });
  }
}
...
function verifyPackage() {
  verifyProof();
  check(existsSync(packageRoot), `missing package root ${packageRelative}; run npm run grafana:generate-package`);
  check(existsSync(receiptPath), "missing installer package receipt; run npm run grafana:generate-package");
  ...
  check(installer.metadata.name === "grafana-grafana", "package name mismatch");
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), "grafana-package-verify-"));
  try {
    const firstPackage = join(tempRoot, "grafana-a.tgz");
    const secondPackage = join(tempRoot, "grafana-b.tgz");
    runCub(["installer", "package", packageRoot, "-o", firstPackage]);
    runCub(["installer", "package", packageRoot, "-o", secondPackage]);
    const firstSHA = sha256File(firstPackage);
    const secondSHA = sha256File(secondPackage);
  ...
  console.log("grafana installer package verification passed");
}
...
function verifySetupVariant(tempRoot, variant, receipt) {
  ...
  check(
    JSON.stringify(extraInCub) === JSON.stringify(["v1|Namespace||grafana"]),
    `${variant.name} cub output may add only v1|Namespace||grafana; found ${extraInCub.join(", ")}`,
  );
  const semanticDiffs = [];
...
function pullSource() {
  const tempRoot = mkdtempSync(join(tmpdir(), "grafana-source-"));
  try {
    command("helm", ["pull", "grafana/grafana", "--version", chart.version, "--destination", tempRoot]);
    const packagePath = listFiles(tempRoot).find((path) => path.endsWith(".tgz"));
    command("tar", ["-xzf", packagePath, "-C", tempRoot]);
    const chartRoot = join(tempRoot, "grafana");
...
function renderVariant(variant) {
  const tempRoot = mkdtempSync(join(tmpdir(), "grafana-render-"));
  try {
    const args = [
      "template",
      chart.releaseName,
...
function effectiveValuesDoc(variant, defaultValuesSHA256) {
  if (!variant.valuesText) {
    return {
      ...
      metadata: { name: "grafana-grafana-10.5.15-default" },
      spec: {
  ...
  return {
    apiVersion: "helm-expt.confighub.com/v1alpha1",
    kind: "EffectiveValues",
    metadata: { name: `grafana-grafana-10.5.15-${variant.name}` },
    spec: {
...
function writeReadme(summaries) {
  write(
    ...
    `# grafana/grafana ${chart.version} Proof
