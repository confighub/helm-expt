...
if (mode === "--generate-proof") {
  ...
  console.log(`Usage:
  node scripts/mongodb-proof.mjs --generate-proof
  node scripts/mongodb-proof.mjs --generate-package
  node scripts/mongodb-proof.mjs --verify-proof
  node scripts/mongodb-proof.mjs --verify-proof-self-test
  node scripts/mongodb-proof.mjs --verify-package
  node scripts/mongodb-proof.mjs --compare`);
}
...
function generateProof() {
    ...
    spec: {
      points: [
        ...
        },
        {
          ...
          note: "The generated-passwords variant binds the generated root password before render so Helm output is deterministic.",
        },
        {
  ...
  writeYaml(join(proofRoot, "recipe.yaml"), {
    apiVersion: "helm-expt.confighub.com/v1alpha1",
    kind: "Recipe",
    metadata: { name: "bitnami-mongodb", version: chart.version },
    spec: {
  ...
  for (const variant of variants) {
    ...
    const inventory = {
      ...
      metadata: { name: `bitnami-mongodb-${chart.version}-${variant.name}-r001` },
      spec: {
    ...
    writeYaml(join(receiptsRoot, "render-receipt.yaml"), {
      apiVersion: "helm-expt.confighub.com/v1alpha1",
      kind: "RenderReceipt",
      metadata: { name: `mongodb-${variant.name}-r001` },
      spec: {
    ...
    writeYaml(join(receiptsRoot, "helm-equivalence-receipt.yaml"), {
      ...
      spec: {
        ...
        classifications: [
          { identity: "v1|Namespace||mongodb", classification: "installer-support-object", disposition: "allowed" },
        ],
        result: "pass",
        evidenceCommand: "npm run mongodb:compare",
      },
    });
  ...
  writeYaml(join(packageRoot, "installer.yaml"), {
    ...
    spec: {
      bases: variants.map((variant, index) => ({
        ...
        description: `mongodb ${variant.displayName} variant rendered from bitnami/mongodb@${chart.version}`,
      })),
    },
  });
  write(
    join(packageRoot, "README.md"),
    `# bitnami/mongodb ${chart.version} Installer Package

This package is generated from the mongodb proof artifacts.
  ...
  try {
    ...
    writeYaml(receiptPath, {
      ...
      spec: {
        ...
        deterministicBundle: {
          command: `cub install package ${packageRelative} -o <tmp>/mongodb-19.0.7.tgz`,
          sha256: firstSHA,
        ...
        setupChecks: variants.map((variant) => ({
          ...
          base: variant.base,
          command: `cub install setup --pull ${packageRelative} --base ${variant.base} --work-dir <tmp> --non-interactive --namespace mongodb`,
          ...
          allowedCubOnlyObjects: ["v1|Namespace||mongodb"],
        })),
      },
    });
  } finally {
...
function verifyProof(root = proofRoot) {
  const requiredFiles = [
    ...
    "effective-values-existing-secret-replicaset.yaml",
    "recipe.yaml",
    "variants/generated-passwords/variant.yaml",
    "variants/existing-secret-replicaset/variant.yaml",
    ...
    "revisions/existing-secret-replicaset/r001/variant-revision.yaml",
    "revisions/existing-secret-replicaset/r001/rendered/release-objects.yaml",
    "revisions/existing-secret-replicaset/r001/rendered/object-inventory.yaml",
    "revisions/existing-secret-replicaset/r001/receipts/helm-equivalence-receipt.yaml",
    "revisions/existing-secret-replicaset/r001/receipts/render-receipt.yaml",
    "revisions/existing-secret-replicaset/r001/receipts/scan-receipt.yaml",
    "revisions/existing-secret-replicaset/r001/receipts/install-gate.yaml",
  ];
  ...
  console.log("verified mongodb proof artifacts");
}

function verifyProofSelfTest() {
  const tempRoot = mkdtempSync(join(tmpdir(), "mongodb-proof-self-test-"));
  try {
    ...
    console.log("self-test passed: mongodb rendered object tampering is rejected");
  } finally {
    rmSync(tempRoot, { recursive: true, force: true });
  }
}
...
function verifyPackage() {
  verifyProof();
  check(existsSync(packageRoot), `missing package root ${packageRelative}; run npm run mongodb:generate-package`);
  check(existsSync(receiptPath), "missing installer package receipt; run npm run mongodb:generate-package");
  ...
  check(installer.metadata.name === "bitnami-mongodb", "package name mismatch");
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), "mongodb-package-verify-"));
  try {
    const firstPackage = join(tempRoot, "mongodb-a.tgz");
    const secondPackage = join(tempRoot, "mongodb-b.tgz");
    runCub(["install", "package", packageRoot, "-o", firstPackage]);
    runCub(["install", "package", packageRoot, "-o", secondPackage]);
    const firstSHA = sha256File(firstPackage);
    const secondSHA = sha256File(secondPackage);
  ...
  console.log("mongodb installer package verification passed");
}
...
function verifySetupVariant(tempRoot, variant, receipt) {
  ...
  check(
    JSON.stringify(extraInCub) === JSON.stringify(["v1|Namespace||mongodb"]),
    `${variant.name} cub output may add only v1|Namespace||mongodb; found ${extraInCub.join(", ")}`,
  );
  const semanticDiffs = [];
...
function pullSource() {
  const tempRoot = mkdtempSync(join(tmpdir(), "mongodb-source-"));
  try {
    command("helm", ["pull", "bitnami/mongodb", "--version", chart.version, "--destination", tempRoot]);
    const packagePath = listFiles(tempRoot).find((path) => path.endsWith(".tgz"));
    command("tar", ["-xzf", packagePath, "-C", tempRoot]);
    const chartRoot = join(tempRoot, "mongodb");
...
function renderVariant(variant) {
  const tempRoot = mkdtempSync(join(tmpdir(), "mongodb-render-"));
  try {
    const args = [
      "template",
      chart.releaseName,
...
function writeReadme(summaries) {
  write(
    ...
    `# bitnami/mongodb ${chart.version} Proof
