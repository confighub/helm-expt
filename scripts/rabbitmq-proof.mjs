...
const proofRoot = join(repoRoot, "recipes", "bitnami", "rabbitmq", "16.0.14");
const packageRoot = join(repoRoot, "packages", "bitnami", "rabbitmq", "16.0.14");
...
if (mode === "--generate-proof") {
  ...
  console.log(`Usage:
  node scripts/rabbitmq-proof.mjs --generate-proof
  node scripts/rabbitmq-proof.mjs --generate-package
  node scripts/rabbitmq-proof.mjs --verify-proof
  node scripts/rabbitmq-proof.mjs --verify-proof-self-test
  node scripts/rabbitmq-proof.mjs --verify-package
  node scripts/rabbitmq-proof.mjs --compare`);
}
...
function generateProof() {
  ...
  writeYaml(join(proofRoot, "source-lock.yaml"), {
    ...
    kind: "SourceLock",
    metadata: { name: "bitnami-rabbitmq-16.0.14" },
    spec: {
      ...
      evidence: {
        harnessReceipt: "../../../../data/adversarial10/charts/bitnami-rabbitmq-16.0.14/render-receipt.yaml",
      },
    },
  });
    ...
    spec: {
      points: [
        ...
        },
        {
          ...
          note: "The generated-passwords variant binds the generated password and Erlang cookie before render so Helm output is deterministic.",
        },
        {
  ...
  writeYaml(join(proofRoot, "recipe.yaml"), {
    apiVersion: "helm-expt.confighub.com/v1alpha1",
    kind: "Recipe",
    metadata: { name: "bitnami-rabbitmq", version: chart.version },
    spec: {
  ...
  for (const variant of variants) {
    ...
    const inventory = {
      ...
      metadata: { name: `bitnami-rabbitmq-${chart.version}-${variant.name}-r001` },
      spec: {
    ...
    writeYaml(join(receiptsRoot, "render-receipt.yaml"), {
      apiVersion: "helm-expt.confighub.com/v1alpha1",
      kind: "RenderReceipt",
      metadata: { name: `rabbitmq-${variant.name}-r001` },
      spec: {
    ...
    writeYaml(join(receiptsRoot, "helm-equivalence-receipt.yaml"), {
      ...
      spec: {
        ...
        classifications: [
          { identity: "v1|Namespace||rabbitmq", classification: "installer-support-object", disposition: "allowed" },
        ],
        result: "pass",
        evidenceCommand: "npm run rabbitmq:compare",
      },
    });
  ...
  writeYaml(join(proofRoot, "chart-dossier.yaml"), {
    ...
    spec: {
      ...
      maintainedNotes: [
        ...
        "configuration, advancedConfiguration, initScripts, secret names, and extraDeploy are template-powered extension slots; promoted variants keep raw slots empty.",
      ],
      knownControlPoints: [
        "generated-facts",
        "target-facts",
...
This package is generated from the rabbitmq proof artifacts.
...
`,
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), "rabbitmq-installer-package-"));
  try {
    ...
    writeYaml(receiptPath, {
      ...
      spec: {
        ...
        setupChecks: variants.map((variant) => ({
          ...
          allowedCubOnlyObjects: ["v1|Namespace||rabbitmq"],
        })),
      },
    });
  } finally {
...
function verifyProof(root = proofRoot) {
  ...
  check(sourceLock.spec.chart === "rabbitmq", "source chart mismatch");
  check(sourceLock.spec.version === "16.0.14", "source version mismatch");
  ...
  console.log("verified rabbitmq proof artifacts");
}

function verifyProofSelfTest() {
  const tempRoot = mkdtempSync(join(tmpdir(), "rabbitmq-proof-self-test-"));
  try {
    ...
    console.log("self-test passed: rabbitmq rendered object tampering is rejected");
  } finally {
    rmSync(tempRoot, { recursive: true, force: true });
  }
}
...
function verifyPackage() {
  verifyProof();
  check(existsSync(packageRoot), `missing package root ${packageRelative}; run npm run rabbitmq:generate-package`);
  check(existsSync(receiptPath), "missing installer package receipt; run npm run rabbitmq:generate-package");
  ...
  check(installer.metadata.name === "bitnami-rabbitmq", "package name mismatch");
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), "rabbitmq-package-verify-"));
  try {
  ...
  console.log("rabbitmq installer package verification passed");
}
...
function verifySetupVariant(tempRoot, variant, receipt) {
  ...
  check(
    JSON.stringify(extraInCub) === JSON.stringify(["v1|Namespace||rabbitmq"]),
    `${variant.name} cub output may add only v1|Namespace||rabbitmq; found ${extraInCub.join(", ")}`,
  );
  const semanticDiffs = [];
...
function pullSource() {
  const tempRoot = mkdtempSync(join(tmpdir(), "rabbitmq-source-"));
  try {
    command("helm", ["pull", "bitnami/rabbitmq", "--version", chart.version, "--destination", tempRoot]);
    const packagePath = listFiles(tempRoot).find((path) => path.endsWith(".tgz"));
    command("tar", ["-xzf", packagePath, "-C", tempRoot]);
    const chartRoot = join(tempRoot, "rabbitmq");
...
function renderVariant(variant) {
  const tempRoot = mkdtempSync(join(tmpdir(), "rabbitmq-render-"));
  try {
    const args = [
      "template",
      chart.releaseName,
...
function effectiveValuesDoc(variant, defaultValuesSHA256) {
  if (!variant.valuesText) {
    return {
      ...
      metadata: { name: "bitnami-rabbitmq-16.0.14-default" },
      spec: {
  ...
  return {
    apiVersion: "helm-expt.confighub.com/v1alpha1",
    kind: "EffectiveValues",
    metadata: { name: `bitnami-rabbitmq-16.0.14-${variant.name}` },
    spec: {
...
function writeReadme(summaries) {
  write(
    ...
    `# bitnami/rabbitmq ${chart.version} Proof
