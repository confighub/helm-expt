...
const HELM_EXPT_REPO_URL = "https://github.com/confighub/helm-expt";
const CONFIGHUB_RENDERING_DESIGN_ISSUE_URL = "https://github.com/confighubai/confighub/issues/4369";
const DEMO_ORG_ORIGINAL_SNAPSHOT = {
  asOf: "2026-06-30",
  spaces: "29 Spaces",
  units: "about 530 Units",
  links: "about 830 Links",
};
const DEMO_ORG_LINK_SNAPSHOT = {
  asOf: "2026-07-03",
  links: "977/1,000 Links",
};
...
<body>
  <header class="home-hero human-hero">
    ...
    <div class="hero-copy">
      ...
      <p class="audience-frame"><strong>What this is:</strong> This is an experimental proof catalog, not a finished product page. It tests whether popular public Helm charts can be rendered once into plain files, then managed as data after that. The proven parts are the recorded chart inputs, rendered YAML, package refs, verification scripts, and selected live receipts linked from this repo. The sketches are named as sketches when the product model is still being designed or the proof is not broad enough yet. Start with <a href="./demo-org.html">the demo org</a> or the <a href="${HELM_EXPT_REPO_URL}#readme">GitHub repo</a> if you want to check a claim; this work also informs the ConfigHub rendering design discussion in <a href="${CONFIGHUB_RENDERING_DESIGN_ISSUE_URL}">#4369</a>.</p>
    </div>
...
<body>
  ...
  <main>
    ...
    <section aria-labelledby="check-claim">
      <h2 id="check-claim">Check one claim</h2>
      <p>The ConfigHub organization is named <code>helm-catalog</code>. It is member-visible today. If you are not a member, check the committed evidence instead: <a href="${HELM_EXPT_REPO_URL}/tree/main/data/helm-org">data/helm-org</a>, <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/oci-hook-delivery-proof">hook delivery proof</a>, <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/crd-ordering-gap">CRD ordering proof</a>, and <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/promote-silent-skip-proof">promotion same-map proof</a>.</p>
    </section>
    <section aria-labelledby="what">
      <h2 id="what">What is in the org</h2>
      <p>Every base variant below is a root Space: the rendered objects as Units, one per manifest, with links inferred between them. Each Space carries its <code>recipe</code> unit (the source record: chart, version, values profile, routing intent) beside the objects it produced, labels for Component, Variant, ChartVersion and routes, and live validation checks. The first published snapshot, as of ${DEMO_ORG_ORIGINAL_SNAPSHOT.asOf}, held ${DEMO_ORG_ORIGINAL_SNAPSHOT.spaces}, ${DEMO_ORG_ORIGINAL_SNAPSHOT.units}, and ${DEMO_ORG_ORIGINAL_SNAPSHOT.links}. The later sketch update, as of ${DEMO_ORG_LINK_SNAPSHOT.asOf}, measured ${DEMO_ORG_LINK_SNAPSHOT.links} after link-copy corrections.</p>
      ${markdownLikeTable([
    ...
    <section aria-labelledby="exhibits">
      ...
      <p><strong>The fleet.</strong> <code>bitnami-nginx-fleet-dev</code>, <code>-staging</code>, <code>-prod-us</code>, <code>-prod-eu</code>. One base change, promoted to three environments; prod-eu deliberately still needs its upgrade, so fleet queries return a real pending release. Both prod Spaces carry delete and destroy gates. The fleet also holds a live registry migration: the base's nginx image was repointed from the public registry to an internal mirror with the digest intact, promoted to dev, staging, and prod-us, and prod-eu left on the old registry. Read the image host in each environment and you can see exactly which have moved. This is the migration a catalog change of hands forces, done as recorded config; the <a href="./d/docs/user/image-registry-migration.html">walkthrough</a> has the commands.</p>
      ...
      <p><strong>The CRD split.</strong> The two argo-cd Spaces show CRDs bundled and separated; the no-crds root is the contract that the cluster owns them. The contract's teeth were re-proven live on ${DEMO_ORG_LINK_SNAPSHOT.asOf}: applying a bundle whose custom resource precedes its CRD fails with the named Kubernetes error, and the CRD-first-with-wait fix succeeds. The committed <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/crd-ordering-gap">CRD ordering proof</a> carries the observation.</p>
      <p><strong>The hooks.</strong> <code>hook-probe-base</code> holds a Job whose Argo hook annotations are readable in the unit data: a routing choice as configuration, not as tribal knowledge. This is not just readable, it ran on ${DEMO_ORG_LINK_SNAPSHOT.asOf}: the same fixture was published once to the org's OCI registry and pulled by Argo CD, Flux, and a no-controller direct applier on a live cluster, with the hook Job observed completing under each. All three legs pass; the committed <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/oci-hook-delivery-proof">hook delivery proof</a> names the rig, commands, and observations.</p>
      <p><strong>How departures meet releases.</strong> <code>hashicorp-vault-demo-base</code> and its three environments, built to answer one question precisely: what happens when an environment has local changes and the base ships a release? Dev tagged itself for cost attribution, staging went to 2 replicas, prod went to 3. Then the base released a telemetry annotation and a release-track stamp, and each environment promoted. Staging and prod show the good case: the releases arrived as <code>UpgradeUnit</code> revisions and the replica departures survived. Dev shows the case to know about: its departure edited the same annotations map the releases wrote to, and on that unit promote kept dev's version and reported success, so the releases never arrived; dev adopted them with two explicit, recorded reconcile commits instead. Field-level departures merge; same-map departures are yours to reconcile. The committed <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/promote-silent-skip-proof">promotion same-map proof</a> shows exactly which happened.</p>
    ...
    <section aria-labelledby="live-proof">
      ...
      <p><strong>The hook delivery proof.</strong> The claim is that ConfigHub publishes a bundle <em>once</em> to its OCI registry, and the delivery tool is a free choice, not a fork in behaviour. To test it, the hook fixture (a workload ConfigMap plus a migration Job carrying Argo hook annotations) was published from this org to the org's OCI registry, and three consumers pulled <em>the same artifact</em> on a throwaway kind cluster. Argo CD used an OCI-sourced Application. Flux used an OCIRepository and a Kustomization. A no-controller path pulled the bundle and applied it directly. Under each of the three, the workload landed <em>and the hook Job ran to completion on the cluster</em>. All three legs pass. This does not prove every chart, every hook shape, or production scale. It proves the transport claim for the routed fixture, on one rig, on ${DEMO_ORG_LINK_SNAPSHOT.asOf}. The <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/oci-hook-delivery-proof">receipt directory</a> names the rig, the times, and each observation.</p>
      <p><strong>The CRD-ordering proof.</strong> This claim is a limitation, stated honestly. The no-controller path has no dependency ordering, so on a first install of a bundle where a custom resource precedes its CRD, plain apply fails. The run reproduced that failure on ${DEMO_ORG_LINK_SNAPSHOT.asOf}. The custom resource was refused with the named Kubernetes error and never created. The fix, CRDs first with a wait for them to establish, made the same bundle succeed. We record this as <em>watch</em>, not pass, because the gap is the finding. It is also why the catalog routes CRD-carrying charts to Argo or Flux, which order and retry, and why the no-crds base variants exist as a contract that the cluster owns its CRDs. The <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/crd-ordering-gap">receipt directory</a> carries the failure and fix.</p>
      <p class="quiet-line">The live receipt directories are committed in the repo: <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/oci-hook-delivery-proof">runs/oci-hook-delivery-proof</a>, <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/crd-ordering-gap">runs/crd-ordering-gap</a>, and <a href="${HELM_EXPT_REPO_URL}/tree/main/runs/promote-silent-skip-proof">runs/promote-silent-skip-proof</a>. The org builder and snapshot data are under <a href="${HELM_EXPT_REPO_URL}/tree/main/data/helm-org">data/helm-org</a>. The rig was torn down after the run; nothing in the org depends on it.</p>
    </section>
    ...
    <section aria-labelledby="sketches">
      ...
      <p>A route exists for anything in a chart that does not survive a config-only render. That is the behaviour Helm hides in hooks, weights, and cluster lookups. In the ${DEMO_ORG_LINK_SNAPSHOT.asOf} sketch, kube-prometheus-stack carries seven routes. Five are hook-derived; the last two show that routes are a bigger idea than hooks.</p>
      ${markdownLikeTable([
      ...
      <p class="quiet-line">In the ${DEMO_ORG_LINK_SNAPSHOT.asOf} sketch, execution modes split four <em>user-executes</em> (a person or agent runs the step and keeps the receipt) and three <em>target-owned</em> (the delivery target's own mechanics carry the behaviour). Every route lists its alternatives and its evidence in the unit data; nothing in the set is marked automatic.</p>
    </section>

    <section aria-labelledby="checks">
      <h2 id="checks">The checks are live, and honest</h2>
      <p>As of ${DEMO_ORG_LINK_SNAPSHOT.asOf}, every Space ran schema and placeholder validation as blocking checks: zero correctness gates across the org. Two advisory checks surfaced real quality findings as warnings: images not pinned by digest, and containers without probes. Those warnings match open work the catalog already tracks; the org shows known debt instead of hiding it. The four production Spaces additionally required approval before apply, so their units carried standing approval gates until someone approved a release. Non-production Spaces and the sketch units deliberately carried none, because no human workflow stood behind approving them. Each Space also wore its recorded proof lanes as labels (render parity, ConfigHub proof, GitOps live), mirrored verbatim from the committed receipts. And the repo checks the org the same way it checks every other surface: <code>npm run helm-org:verify</code> compares the live org, including each Space's recipe unit, against the committed catalog. A check that does not reflect a real validation would be a lie in the UI, so there are none of those.</p>
      <p class="quiet-line">Receipts and the tool that builds all of this are committed in the repo under <a href="${HELM_EXPT_REPO_URL}/tree/main/data/helm-org">data/helm-org</a>; the org is regenerable and drift-checkable like every other catalog surface. The org itself is member-visible today; these pages, the receipts, and the walkthroughs are the public record of it.</p>
    </section>
