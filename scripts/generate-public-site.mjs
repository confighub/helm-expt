...
<body>
  ...
  <main>
    ...
    <section aria-labelledby="exhibits">
      ...
      <p><strong>The sketches of the unbuilt.</strong> Four things this catalog talks about have no product entity yet: the recipe, the act of rendering, render provenance, and lifecycle routes. The org sketches each one with today's primitives, the same move as the <code>recipe</code> unit. <code>hashicorp-vault-demo-base</code> carries a <code>render-record</code> unit recording what the client-side render did (chart, base, when, by what, how many units came out), and a <code>rendered-from-recipe</code> Link from the statefulset to the recipe unit, one exemplar of the provenance edge the product would draw for every unit. <code>route-sketch-kube-prometheus-stack</code> holds seven <code>LifecycleRoute</code> units, one per recorded kube-prometheus-stack route, each addressable, queryable, and honestly <code>automatic: false</code>. When these become first-class product objects, the sketches collapse into them; until then the gap is visible instead of buried.</p>
    </section>

    <section aria-labelledby="checks">
      <h2 id="checks">The checks are live, and honest</h2>
      <p>Every Space runs schema and placeholder validation as blocking checks: zero correctness gates across the org. Two advisory checks surface real quality findings as warnings: images not pinned by digest, and containers without probes. Those warnings match open work the catalog already tracks; the org shows known debt instead of hiding it. The three production Spaces additionally require approval before apply, so their units carry standing approval gates until someone approves a release. Each Space also wears its recorded proof lanes as labels (render parity, ConfigHub proof, GitOps live), mirrored verbatim from the committed receipts. And the repo checks the org the same way it checks every other surface: <code>npm run helm-org:verify</code> compares the live org, including each Space's recipe unit, against the committed catalog. A check that does not reflect a real validation would be a lie in the UI, so there are none of those.</p>
