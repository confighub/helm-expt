...
const chart = "external-secrets/external-secrets";
const chartSlug = "external-secrets-external-secrets";
const version = "2.5.0";
...
if (mode === "--generate") {
  ...
  console.log(`wrote External Secrets support artifacts -> ${relativeRepo(outputRoot)}/`);
} else if (mode === "--verify") {
  ...
  console.log("verified External Secrets support artifacts");
} else {
  console.log(`Usage:
  node scripts/generate-eso-production-support-artifacts.mjs --generate
  node scripts/generate-eso-production-support-artifacts.mjs --verify`);
}
  ...
  return {
    ...
    spec: {
      ...
      productionSupportUse: {
        ...
        claim: "The mutable rendered image references for this candidate base were resolved to registry manifest digests at the recorded time.",
        ...
        nextRequired: "Choose a digest-pinned base, image override policy, or explicit mutable-image exception before final production OCI support.",
      },
    },
  };
}
...
function buildImagePolicyDecision() {
  ...
  return {
    ...
    spec: {
      ...
      claim:
        "The rendered External Secrets bases still contain mutable image tags, but each rendered image reference has digest-resolution evidence. For this public controller support scope, the mutable tag is accepted as a target-scoped exception while stricter environments remain free to require digest-pinned bases or image overrides.",
      ...
      acceptedException: {
        reason:
          "The public proof target uses the upstream chart image tag for a controller install. The receipt set records the digest that the mutable tag resolved to at decision time, so reviewers can rerun the decision when the upstream tag moves.",
        operatorAction:
      ...
      evidence: [
        { path: relativeRepo(imageWorkdownPath), claim: "Summarizes External Secrets rendered image references." },
        { path: relativeRepo(imageReviewPath), claim: "Lists every rendered image reference found in the External Secrets object sets." },
        ...receipts.map((receipt) => ({
          path: receipt.path,
...
function buildSecurityDecision() {
  ...
  return {
    ...
    spec: {
      ...
      decidedAt: "2026-06-09",
      claim:
        "The recorded kube-linter findings are missing CPU and memory requests/limits on the External Secrets controller workloads. They are accepted for this public cub-lk controller proof scope. Stricter environments should create a resource-policy base or overlay before support expansion.",
      ...
      acceptedFindings: [
        {
          ...
          disposition:
            "Accepted for the public cub-lk proof scope. Add resource requests in a hardened base for customer or production SLO scopes.",
        },
        {
      ...
      evidence: [
        { path: relativeRepo(scanWorkdownPath), claim: "Routes External Secrets scan findings to add-resource-policy." },
  ...
  return {
    ...
    spec: {
      ...
      observedLifecycleSignals: {
        ...
        controllerOwnedFields: lifecycle.spec.controllerOwnedFields,
      ...
      evidence: [
        { path: relativeRepo(lifecyclePath), claim: "Lifecycle observation for the default base." },
        {
          path: `runs/lifecycle-observations/cert-manager-eso/${chartSlug}-no-crds/receipt.yaml`,
  ...
  return {
    ...
    spec: {
      ...
      supportClaim: {
        ...
        detail: "Fresh target-scoped ConfigHub OCI and Argo evidence passed for the declared External Secrets default support scope.",
      },
      limits: [
...
function verifyImageReceipt(variant) {
  ...
  check((receipt.spec.images ?? []).length === 1, `${receipt.path} expected one unique image`);
...
function verifyImagePolicyDecision() {
  ...
  check(decision.spec.decision === "mutable-image-exception-accepted-for-target-scope", "ESO image policy decision mismatch");
  check((decision.spec.variantsCovered ?? []).length === variants.length, "ESO image policy variants mismatch");
}

function verifySecurityDecision() {
  const decision = decisionFile("security-decision.yaml", "ProductionSecurityDecision");
  check(decision.spec.decision === "resource-policy-accepted-for-target-scope", "ESO security decision mismatch");
  check(decision.spec.findingSummary.totalFindings === Number(scanWorkdownRow().findingCount), "ESO security finding count mismatch");
  for (const row of externalScanRows()) {
    check(decision.spec.findingSummary.variants[row.variant].renderedObjectSetSHA256 === row.renderedSHA256, `ESO security rendered sha mismatch for ${row.variant}`);
  }
}
...
function verifyFreshEvidenceReceipt() {
  ...
  const live = liveReceipt();
  check(receipt.spec.supportClaim.state === "fresh-target-evidence-passed", "ESO fresh evidence claim mismatch");
  check(receipt.spec.observedAt === live.spec.observedAt, "ESO fresh evidence observedAt mismatch");
...
function imageReceipt(variant) {
  ...
  check(existsSync(path), `missing ${relativeRepo(path)}; run npm run eso:production-support`);
...
function imageWorkdownRow() {
  ...
  check(rows.length === 1, "expected one ESO image workdown row");
  return rows[0];
}
...
function externalScanRows() {
  ...
  check(rows.length === variants.length, "expected ESO external scan rows");
  return rows;
}
...
function scanWorkdownRow() {
  ...
  check(rows.length === 1, "expected one ESO scan workdown row");
  return rows[0];
}

function lifecycleReceipt() {
  check(existsSync(lifecyclePath), `missing ${relativeRepo(lifecyclePath)}`);
  const receipt = readYaml(lifecyclePath);
  check(receipt.kind === "LifecycleObservationReceipt", `${relativeRepo(lifecyclePath)} kind mismatch`);
  check(receipt.spec?.chart === chart, `${relativeRepo(lifecyclePath)} chart mismatch`);
  check(receipt.spec?.version === version, `${relativeRepo(lifecyclePath)} version mismatch`);
  check(receipt.spec?.base === supportedBase, `${relativeRepo(lifecyclePath)} base mismatch`);
  check(receipt.spec?.result === "pass", `${relativeRepo(lifecyclePath)} result must pass`);
  return receipt;
}
