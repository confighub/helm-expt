...
function summaryMd(intents, matrixRows, candidates) {
  ...
  o.push("");
  o.push("This is the bridge between the full helm-expt chain and the simpler ConfigHub model. A render intent is one reviewed base-variant path: chart, version, values profile, namespace, capability profile, source lock, package base, evidence lanes, lifecycle routes, and target prerequisites.");
  o.push("");
  o.push("It is intentionally generated only for real base rows in the master matrix. Candidate and custom-discussion rows stay visible in the matrix, but they are not emitted as runnable render-intent configs yet.");
  o.push("");
  o.push("## Snapshot", "");
