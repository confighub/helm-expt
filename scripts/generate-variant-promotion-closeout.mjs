...
//
// Turns the master matrix's promotion column (0 proven / 20 watch / 172 todo)
// into a product-and-engineering queue: for every variant, what its promotion
// state is, whether a server-side variant clone already exists, whether
// `cub variant promote` is ready-to-run now / watch-grade / blocked by the known
// changeset add-new-units bug, the exact next command or fix, and the owner class
// that has to act.
//
...
// promote today, which are watch-grade, and which need the ConfigHub server fix.
//
...
const DECISIONS = {
  ...
  "proven-with-watch": {
    ...
    owner_class: "fix-confighub-server",
    server_clone_exists: "yes",
    blocked_by: CHANGESET_BUG,
    meaning:
      "Server-side promotion mechanics passed, but the changeset-bound (add-new-units) promote path fails on the known ConfigHub server bug. The basic promote is proven; the changeset path needs the server fix.",
  },
  ...
  return `${JSON.stringify(
    {
      ...
      unofficial: true,
      description:
        "Promotion-cell closeout queue: one row per variant with promotion state, whether a server-side clone exists, whether cub variant promote is ready-to-run / watch-grade / blocked by the changeset add-new-units bug, the exact next command or fix, and the owner class. Promotion is a ConfigHub server value. Read-only projection; generated by scripts/generate-variant-promotion-closeout.mjs; do not hand-edit.",
...
function buildSummary(rows) {
  ...
  lines.push("");
  lines.push("The master matrix promotion column reads **0 proven / 20 watch / 172 todo**.");
  lines.push("This surface turns that into a product-and-engineering queue: for every variant,");
  lines.push("whether `cub variant promote` is **ready to run now**, **watch-grade**, or");
  lines.push("**blocked** by the known ConfigHub server bug — with the exact next command or fix");
  ...
  lines.push("| --- | ---: | --- |");
  lines.push(`| \`run-proof\` | ${rows.filter((r) => r.owner_class === "run-proof").length} | A clone exists (or a prerequisite proof can run); record the proof. Engineering/CI. |`);
  lines.push(`| \`fix-confighub-server\` | ${rows.filter((r) => r.owner_class === "fix-confighub-server").length} | Blocked on the ConfigHub server changeset add-new-units bug (${CHANGESET_BUG}). |`);
  ...
  lines.push("");
  lines.push(`## Watch-grade — changeset path blocked by the server bug (${watch.length})`);
  lines.push("");
  lines.push(`Server-side promotion mechanics are proven for these, but the changeset-bound`);
  lines.push(`(add-new-units) promote path fails on ${CHANGESET_BUG}. They are usable for the`);
  lines.push("proven mechanics; the changeset path needs the ConfigHub server fix.");
  lines.push("");
  ...
  lines.push("- A `watch-grade` row is a recorded decision (mechanics proven, changeset path");
  lines.push("  blocked), never silently rounded to proven.");
  lines.push("");
  return `${lines.join("\n")}`;
}
