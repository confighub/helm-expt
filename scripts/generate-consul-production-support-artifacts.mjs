...
const chart = "hashicorp/consul";
const chartSlug = "hashicorp-consul";
const version = "2.0.0";
const supportedBase = "default-control-plane";
const variants = ["default-control-plane", "secure-mesh-existing-secrets"];
...
if (mode === "--generate") {
  ...
  console.log(`wrote Consul support artifacts -> ${relativeRepo(outputRoot)}/`);
} else if (mode === "--verify") {
  ...
  console.log("verified Consul support artifacts");
} else {
  console.log(`Usage:
  node scripts/generate-consul-production-support-artifacts.mjs --generate
  node scripts/generate-consul-production-support-artifacts.mjs --verify`);
}
...
function buildImagePolicyDecision() {
  ...
  return {
    ...
    spec: {
      ...
      claim:
        "The rendered Consul bases still contain mutable image tags, but each rendered image reference has digest-resolution evidence. For the default control-plane public proof scope, the mutable tags are accepted as a target-scoped exception while stricter environments remain free to require digest-pinned bases or image overrides.",
      ...
      acceptedException: {
        reason:
          "The public proof target uses upstream Consul image tags. The receipt set records the registry digest each tag resolved to at decision time, so reviewers can rerun the decision when upstream tags move.",
        operatorAction:
      ...
      evidence: [
        { path: relativeRepo(imageWorkdownPath), claim: "Summarizes Consul rendered image references." },
        { path: relativeRepo(imageReviewPath), claim: "Lists every rendered image reference found in the Consul object sets." },
        ...receipts.map((receipt) => ({
          path: receipt.path,
    ...
    spec: {
      ...
      acceptedFindings: [
        ...
        },
        {
          ...
          disposition:
            "Accepted for the public proof scope. Keep or adjust probe port naming according to target policy in a stricter base.",
        },
      ],
      topologyDisposition:
        "default-control-plane is the supported proof topology. secure-mesh-existing-secrets remains review input for TLS, ACL, gateway, UI, and existing-Secret operation, but it is not production-supported here.",
      limits: [
      ...
      evidence: [
        { path: relativeRepo(scanWorkdownPath), claim: "Routes Consul scan findings to add-resource-policy." },
        { path: relativeRepo(externalScanPath), claim: "Records kube-linter warning counts for Consul rendered object sets." },
...
function buildLifecycleDecision() {
  ...
  return {
    ...
    spec: {
      ...
      decidedAt: "2026-06-09",
      claim:
        "The Consul default-control-plane base is managed as reviewed desired Kubernetes objects with Helm hooks excluded. Regular Helm, cub installer apply, and ConfigHub OCI/Argo all reach runtime readiness for the supported proof scope.",
      lifecycleModel: {
        hookPolicy: "no-hooks-in-promoted-render",
        selectedTopology: "default-control-plane",
        observedWorkloads: ["consul-consul-server", "consul-consul-connect-injector", "consul-consul-webhook-cert-manager"],
        crdPolicy:
          "Consul and Gateway API CRDs are package-owned in this proof scope; platform-owned CRDs need a separate no-crds or external-CRD base.",
        webhookPolicy:
          "Connect injector and webhook certificate manager readiness must be observed fresh for supported targets.",
        secureMeshPolicy:
          "TLS, ACL, gossip, gateway, UI ingress, ACL init, and existing Secret operation belong to secure-mesh-existing-secrets or another separately reviewed base.",
      },
      ...
      evidence: [
        { path: relativeRepo(kindParityPath), claim: "Two-cluster Helm-vs-installer parity passes for default-control-plane." },
        { path: relativeRepo(liveReceiptPath), claim: "ConfigHub OCI/Argo live parity passes for default-control-plane." },
        {
...
function buildFreshEvidenceReceipt() {
  ...
  return {
    ...
    spec: {
      ...
      checks: [
        { name: "regular-helm-runtime", result: live.spec.legs.regularHelm.runtime.result, detail: "regular Helm Consul runtime passed" },
        { name: "confighub-apply-runtime", result: live.spec.legs.configHubKubectlApply.runtime.result, detail: "ConfigHub rendered objects applied by kubectl reached Consul readiness" },
      ...
      supportClaim: {
        ...
        detail: "Fresh target-scoped ConfigHub OCI and Argo evidence passed for the declared Consul default-control-plane support scope.",
      },
      limits: [
        ...
        "This supports the default-control-plane base, not the secure mesh or gateway topology.",
...
function verifyImagePolicyDecision() {
  ...
  check(decision.spec.decision === "mutable-image-exception-accepted-for-target-scope", "Consul image policy decision mismatch");
  check((decision.spec.variantsCovered ?? []).length === variants.length, "Consul image policy variants mismatch");
  check(decision.spec.supportedBaseCandidate === supportedBase, "Consul image policy supported base mismatch");
}

function verifySecurityDecision() {
  const decision = decisionFile("security-decision.yaml", "ProductionSecurityDecision");
  check(decision.spec.decision === "default-control-plane-resource-policy-accepted-for-target-scope", "Consul security decision mismatch");
  check(decision.spec.supportedBaseCandidate === supportedBase, "Consul security supported base mismatch");
  check(decision.spec.findingSummary.totalFindings === Number(scanWorkdownRow().findingCount), "Consul security finding count mismatch");
  for (const row of externalScanRows()) {
    check(decision.spec.findingSummary.variants[row.variant].renderedObjectSetSHA256 === row.renderedSHA256, `Consul security rendered sha mismatch for ${row.variant}`);
  }
}

function verifyLifecycleDecision() {
  const decision = decisionFile("lifecycle-decision.yaml", "ProductionLifecycleDecision");
  check(decision.spec.decision === "lifecycle-observed-for-proof-scope", "Consul lifecycle decision mismatch");
  check(decision.spec.supportedBaseCandidate === supportedBase, "Consul lifecycle supported base mismatch");
  check(decision.spec.lifecycleModel.hookPolicy === "no-hooks-in-promoted-render", "Consul lifecycle hook policy mismatch");
  check(decision.spec.observedLifecycleSignals.confighubOciArgo.argoSync === "Synced", "Consul lifecycle Argo sync mismatch");
  check(decision.spec.observedLifecycleSignals.confighubOciArgo.argoHealth === "Healthy", "Consul lifecycle Argo health mismatch");
}
...
function verifyFreshEvidenceReceipt() {
  ...
  const live = liveReceipt();
  check(receipt.spec.supportClaim.state === "fresh-target-evidence-passed", "Consul fresh evidence claim mismatch");
  check(receipt.spec.observedAt === live.spec.observedAt, "Consul fresh evidence observedAt mismatch");
  check(receipt.spec.oci.revision === live.spec.legs.configHubOciArgo.ociRevision, "Consul fresh evidence OCI revision mismatch");
  check(receipt.spec.oci.sync === "Synced", "Consul Argo sync mismatch");
  check(receipt.spec.oci.health === "Healthy", "Consul Argo health mismatch");
}
...
function imageReceipt(variant) {
  ...
  check(existsSync(path), `missing ${relativeRepo(path)}; run npm run consul:production-support`);
...
function imageWorkdownRow() {
  ...
  check(rows.length === 1, "expected one Consul image workdown row");
  return rows[0];
}
...
function externalScanRows() {
  ...
  check(rows.length === variants.length, "expected Consul external scan rows");
  return rows;
}
...
function scanWorkdownRow() {
  ...
  check(rows.length === 1, "expected one Consul scan workdown row");
  return rows[0];
}
