...
We keep the Helm chart. We lock \`${chart}@${version}\` and choose the \`${base}\` preset config, then render it with the recorded values, namespace and release name against the recorded Kubernetes capabilities, and save the output as files.
...
The public package is \`${packageRef}\`. Users can pull these exact bytes without cloning this repo. When someone runs \`cub installer upload\`, ConfigHub stores the rendered Kubernetes YAML in a Space, where it can be searched and compared, reviewed and changed, then delivered. The example script defaults to Space \`${displaySpace}\`, but users can choose another name with \`CUB_SPACE=...\`.
