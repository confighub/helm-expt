...
const receiptRoot = join(repoRoot, "runs", "metrics-server-use-more-now", "latest");

verifyUseMoreNow();
verifyFunctionScan();
verifySafeOps();

console.log("verified Metrics Server use-more-now scan and safe-ops receipts");

function verifyUseMoreNow() {
  const path = join(receiptRoot, "use-more-now-receipt.yaml");
  check(existsSync(path), "missing Metrics Server use-more-now receipt");
  const receipt = readYaml(path);
  check(receipt.kind === "UseMoreNowReceipt", "Metrics Server use-more-now receipt kind mismatch");
...
function verifyFunctionScan() {
  ...
  check(receipt.spec?.context?.space === "helm-metrics-server-use-more-now", "Metrics Server function scan receipt must target proof space");
...
function verifySafeOps() {
  ...
  check(receipt.spec?.context?.space === "helm-metrics-server-use-more-now", "Metrics Server safe-ops receipt must target proof space");
