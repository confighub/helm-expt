...
if (mode === "--generate-proof") {
  ...
  console.log(`Usage:
  node scripts/postgresql-proof.mjs --generate-proof
  node scripts/postgresql-proof.mjs --generate-package
  node scripts/postgresql-proof.mjs --verify-proof
  node scripts/postgresql-proof.mjs --verify-proof-self-test
  node scripts/postgresql-proof.mjs --verify-package
  node scripts/postgresql-proof.mjs --compare`);
}
...
function generateProof() {
    ...
    spec: {
      checkedValues: [
        ...
        },
        {
          ...
          reason: "PostgreSQL exposes tpl/config extension slots; promoted variants keep them empty",
        },
      ],
    ...
    spec: {
      points: [
        ...
        },
        {
          ...
          note: "The generated-passwords variant binds the generated password before render so Helm output is deterministic.",
        },
        {
  ...
  writeYaml(join(proofRoot, "recipe.yaml"), {
    apiVersion: "helm-expt.confighub.com/v1alpha1",
    kind: "Recipe",
    metadata: { name: "bitnami-postgresql", version: chart.version },
    spec: {
  ...
  for (const variant of variants) {
    ...
    const inventory = {
      ...
      metadata: { name: `bitnami-postgresql-${chart.version}-${variant.name}-r001` },
      spec: {
    ...
    writeYaml(join(receiptsRoot, "render-receipt.yaml"), {
      apiVersion: "helm-expt.confighub.com/v1alpha1",
      kind: "RenderReceipt",
      metadata: { name: `postgresql-${variant.name}-r001` },
      spec: {
    ...
    writeYaml(join(receiptsRoot, "helm-equivalence-receipt.yaml"), {
      ...
      spec: {
        ...
        classifications: [
          { identity: "v1|Namespace||postgresql", classification: "installer-support-object", disposition: "allowed" },
        ],
        result: "pass",
        evidenceCommand: "npm run postgresql:compare",
      },
    });
  ...
  writeYaml(join(packageRoot, "installer.yaml"), {
    ...
    spec: {
      bases: variants.map((variant, index) => ({
        ...
        description: `postgresql ${variant.displayName} variant rendered from bitnami/postgresql@${chart.version}`,
      })),
    },
  });
  write(
    join(packageRoot, "README.md"),
    `# bitnami/postgresql ${chart.version} Installer Package

This package is generated from the postgresql proof artifacts.
  ...
  try {
    ...
    writeYaml(receiptPath, {
      ...
      spec: {
        ...
        deterministicBundle: {
          command: `cub install package ${packageRelative} -o <tmp>/postgresql-18.6.7.tgz`,
          sha256: firstSHA,
        ...
        setupChecks: variants.map((variant) => ({
          ...
          base: variant.base,
          command: `cub install setup --pull ${packageRelative} --base ${variant.base} --work-dir <tmp> --non-interactive --namespace postgresql`,
          ...
          allowedCubOnlyObjects: ["v1|Namespace||postgresql"],
        })),
      },
    });
  } finally {
...
function verifyProof(root = proofRoot) {
  ...
  console.log("verified postgresql proof artifacts");
}

function verifyProofSelfTest() {
  const tempRoot = mkdtempSync(join(tmpdir(), "postgresql-proof-self-test-"));
  try {
    ...
    console.log("self-test passed: postgresql rendered object tampering is rejected");
  } finally {
    rmSync(tempRoot, { recursive: true, force: true });
  }
}
...
function verifyPackage() {
  verifyProof();
  check(existsSync(packageRoot), `missing package root ${packageRelative}; run npm run postgresql:generate-package`);
  check(existsSync(receiptPath), "missing installer package receipt; run npm run postgresql:generate-package");
  ...
  check(installer.metadata.name === "bitnami-postgresql", "package name mismatch");
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), "postgresql-package-verify-"));
  try {
    const firstPackage = join(tempRoot, "postgresql-a.tgz");
    const secondPackage = join(tempRoot, "postgresql-b.tgz");
    runCub(["install", "package", packageRoot, "-o", firstPackage]);
    runCub(["install", "package", packageRoot, "-o", secondPackage]);
    const firstSHA = sha256File(firstPackage);
    const secondSHA = sha256File(secondPackage);
  ...
  console.log("postgresql installer package verification passed");
}
...
function verifySetupVariant(tempRoot, variant, receipt) {
  ...
  check(
    JSON.stringify(extraInCub) === JSON.stringify(["v1|Namespace||postgresql"]),
    `${variant.name} cub output may add only v1|Namespace||postgresql; found ${extraInCub.join(", ")}`,
  );
  const semanticDiffs = [];
...
function pullSource() {
  const tempRoot = mkdtempSync(join(tmpdir(), "postgresql-source-"));
  try {
    command("helm", ["pull", "bitnami/postgresql", "--version", chart.version, "--destination", tempRoot]);
    const packagePath = listFiles(tempRoot).find((path) => path.endsWith(".tgz"));
    command("tar", ["-xzf", packagePath, "-C", tempRoot]);
    const chartRoot = join(tempRoot, "postgresql");
...
function renderVariant(variant) {
  const tempRoot = mkdtempSync(join(tmpdir(), "postgresql-render-"));
  try {
    const args = [
      "template",
      chart.releaseName,
...
function writeReadme(summaries) {
  write(
    ...
    `# bitnami/postgresql ${chart.version} Proof
