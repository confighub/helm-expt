#!/usr/bin/env bash
# traefik/traefik 40.2.0 - base variant: no-crds
...
say "Pull the package and render the no-crds base variant into ./traefik-traefik-40-2-0-no-crds"
...
ls ./traefik-traefik-40-2-0-no-crds/out/manifests

say "Ensure the default namespace exists"
kubectl create namespace default --dry-run=client -o yaml | kubectl apply -f -

if [ -d ./traefik-traefik-40-2-0-no-crds/out/secrets ]; then
  say "Apply rendered Secrets first"
  kubectl apply -f ./traefik-traefik-40-2-0-no-crds/out/secrets
fi

say "Apply the rendered objects"
kubectl apply -f ./traefik-traefik-40-2-0-no-crds/out/manifests

say "Done. The cluster received exactly the files in ./traefik-traefik-40-2-0-no-crds/out."
