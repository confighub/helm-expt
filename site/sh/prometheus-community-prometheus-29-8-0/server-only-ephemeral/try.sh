#!/usr/bin/env bash
# prometheus-community/prometheus 29.8.0 - base variant: server-only-ephemeral
...
if [ -d ./prometheus-community-prometheus-29-8-0-server-only-ephemeral/out/secrets ]; then
  say "Apply rendered Secrets first"
  kubectl apply -f ./prometheus-community-prometheus-29-8-0-server-only-ephemeral/out/secrets
fi

say "Apply the rendered objects"
kubectl apply -f ./prometheus-community-prometheus-29-8-0-server-only-ephemeral/out/manifests

say "Done. The cluster received exactly the files in ./prometheus-community-prometheus-29-8-0-server-only-ephemeral/out."
