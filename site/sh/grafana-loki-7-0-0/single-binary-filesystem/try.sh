#!/usr/bin/env bash
# grafana/loki 7.0.0 - base variant: single-binary-filesystem
...
say "Pull the package and render the single-binary-filesystem base variant into ./grafana-loki-7-0-0-single-binary-filesystem"
...
ls ./grafana-loki-7-0-0-single-binary-filesystem/out/manifests

say "Ensure the loki namespace exists"
kubectl create namespace loki --dry-run=client -o yaml | kubectl apply -f -

if [ -d ./grafana-loki-7-0-0-single-binary-filesystem/out/secrets ]; then
  say "Apply rendered Secrets first"
  kubectl apply -f ./grafana-loki-7-0-0-single-binary-filesystem/out/secrets
fi

say "Apply the rendered objects"
kubectl apply -f ./grafana-loki-7-0-0-single-binary-filesystem/out/manifests

say "Done. The cluster received exactly the files in ./grafana-loki-7-0-0-single-binary-filesystem/out."
