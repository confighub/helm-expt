#!/usr/bin/env bash
# external-dns/external-dns 1.21.1 - preset: no-crds
...
say "Pull the package and render the no-crds preset into ./external-dns-external-dns-1-21-1-no-crds"
...
if [ -d ./external-dns-external-dns-1-21-1-no-crds/out/secrets ]; then
  say "Apply rendered Secrets first"
  kubectl apply -f ./external-dns-external-dns-1-21-1-no-crds/out/secrets
fi

say "Apply the rendered objects"
kubectl apply -f ./external-dns-external-dns-1-21-1-no-crds/out/manifests

say "Done. The cluster received exactly the files in ./external-dns-external-dns-1-21-1-no-crds/out."
