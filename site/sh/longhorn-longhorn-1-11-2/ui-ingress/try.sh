#!/usr/bin/env bash
# longhorn/longhorn 1.11.2 - preset: ui-ingress
# Path: pull the package, render this preset locally, read the objects,
# then apply them with kubectl. No ConfigHub account is needed.
# Generated by scripts/generate-public-site.mjs from the committed package
# data for this preset. Chart page: https://confighub.github.io/helm-expt/site/charts/longhorn-longhorn-1-11-2.html
set -euo pipefail

say() { printf '\n>> %s\n' "$*"; }

if ! command -v cub >/dev/null 2>&1; then
  printf 'cub is not installed. Install it with:\n  curl -fsSL https://hub.confighub.com/cub/install.sh | bash\nthen add ~/.confighub/bin to your PATH and re-run this script.\n' >&2
  exit 1
fi

if ! command -v kubectl >/dev/null 2>&1; then
  printf 'kubectl is required for the apply step.\n' >&2
  exit 1
fi

say "Pull the package and render the ui-ingress preset into ./longhorn-longhorn-1-11-2-ui-ingress"
cub installer setup --pull oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/longhorn-longhorn:1.11.2 --base ui-ingress --work-dir ./longhorn-longhorn-1-11-2-ui-ingress --non-interactive --namespace longhorn-system

say "Read what was rendered; nothing has touched the cluster yet"
ls ./longhorn-longhorn-1-11-2-ui-ingress/out/manifests

say "Ensure the longhorn-system namespace exists"
kubectl create namespace longhorn-system --dry-run=client -o yaml | kubectl apply -f -

if [ -d ./longhorn-longhorn-1-11-2-ui-ingress/out/secrets ]; then
  say "Apply rendered Secrets first"
  kubectl apply -f ./longhorn-longhorn-1-11-2-ui-ingress/out/secrets
fi

say "Apply the rendered objects"
kubectl apply -f ./longhorn-longhorn-1-11-2-ui-ingress/out/manifests

say "Done. The cluster received exactly the files in ./longhorn-longhorn-1-11-2-ui-ingress/out."
