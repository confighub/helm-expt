...
case "$base" in
  'default')
    if [ "$check_mode" = "live" ]; then
      live_check_secret 'default' 'vpa-tls-certs' 'ca'
      live_check_secret 'default' 'vpa-tls-certs' 'cert'
      live_check_secret 'default' 'vpa-tls-certs' 'key'
      result="pass"
    else
      result="recorded"
    fi
...
targetFacts:
  ...
  - deliveryLanes:
    ...
    name: vpa-tls-certs
    namespace: default
    purpose: Webhook serving certificate Secret mounted by the VPA admission controller;
      Helm normally creates this via the certgen hook
    suggestedSource: Run the chart certgen lifecycle action or stage a valid vpa-tls-certs
      Secret before waiting for the admission controller
