...
case "$base" in
  'existing-secret')
    if [ "$check_mode" = "live" ]; then
      live_check_secret 'default' 'rabbitmq-consolidated' 'rabbitmq-erlang-cookie'
      live_check_secret 'default' 'rabbitmq-consolidated' 'rabbitmq-password'
      result="pass"
    else
      result="recorded"
    fi
