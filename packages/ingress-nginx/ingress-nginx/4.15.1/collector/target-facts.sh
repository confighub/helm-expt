...
case "$base" in
  'default')
    if [ "$check_mode" = "live" ]; then
      live_check_secret 'ingress-nginx' 'ingress-nginx-admission' 'cert'
      live_check_secret 'ingress-nginx' 'ingress-nginx-admission' 'key'
      live_check_secret 'ingress-nginx' 'ingress-nginx-admission' 'ca'
      result="pass"
    else
      result="recorded"
    fi
