...
case "$base" in
  'no-crds')
    if [ "$check_mode" = "live" ]; then
      live_check_secret 'default' 'cnpg-webhook-cert' 'tls.crt'
      live_check_secret 'default' 'cnpg-webhook-cert' 'tls.key'
      live_check_crd 'backups.postgresql.cnpg.io'
      live_check_crd 'clusterimagecatalogs.postgresql.cnpg.io'
      live_check_crd 'clusters.postgresql.cnpg.io'
      live_check_crd 'databases.postgresql.cnpg.io'
      live_check_crd 'failoverquorums.postgresql.cnpg.io'
      live_check_crd 'imagecatalogs.postgresql.cnpg.io'
      live_check_crd 'poolers.postgresql.cnpg.io'
      live_check_crd 'publications.postgresql.cnpg.io'
      live_check_crd 'scheduledbackups.postgresql.cnpg.io'
      live_check_crd 'subscriptions.postgresql.cnpg.io'
      result="pass"
    else
      result="recorded"
    fi
...
targetFacts:
  ...
  - deliveryLanes:
    ...
    namespace: default
    purpose: CloudNativePG admission webhook TLS material required before the no-crds
      operator pod can become ready
