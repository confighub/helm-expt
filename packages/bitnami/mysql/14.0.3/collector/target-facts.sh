...
case "$base" in
  'existing-secret')
    if [ "$check_mode" = "live" ]; then
      live_check_secret 'mysql' 'mysql-auth' 'mysql-root-password'
      live_check_secret 'mysql' 'mysql-auth' 'mysql-password'
      live_check_secret 'mysql' 'mysql-auth' 'mysql-replication-password'
      result="pass"
    else
      result="recorded"
    fi
