...
| Target prerequisites and lifecycle routes | Users know what must exist before apply/GitOps can converge: CRDs, Secrets, storage, topology, webhooks, cloud identity, and hooks. | Hooks page, FAQ, chart pages, matrix hard-gap fields, and target-prerequisite guides. | Target-prerequisite action packets, lifecycle-route actions, hook dispositions, live observations for cert-manager/ESO and serious charts. | Product preflight checklist and target-facts UX with machine-readable action packets for CLI, UI, agents, and fleet runs. |
