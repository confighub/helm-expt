...
/ refused* — and for *observed* and *routed* hooks the installer sets the route
up as part of one apply flow, with a receipt that proves it ran. You never
reverse-engineer Helm's engine.
...
Sufficiency requires three things beyond docs and the matrix:

1. **Executed routes, not documented routes.** *observed* and *routed* must
   resolve to operations the installer performs — not instructions. If the
   disposition is a paragraph, the docs are the interface and we lose. If it is
   an operation, the docs are just the explanation.
...
| **routed** | The route is known and wired, but live evidence is not yet captured. | Nothing yet — but knows it is not yet proven. |
...
| --- | --- | --- | --- |
| Hook (pre/post install) | preflight-or-presync / postsync-check | observed / routed | product |
| Hook (upgrade) | upgrade-action-with-receipt | routed | product (gated) |
...
| install-vs-upgrade | upgrade-action-with-receipt | routed | product |
...
**(a) Executed routes.** *observed* and *routed* must be operations:
...
If a route can only be documented, its honest disposition is **per-target** or
**refused** — never a *routed* that secretly means "you do it."
...
## 4. Worked example A — the user's hook, end to end
...
2. **The route page tells you where it goes.** Route = `preflight-or-presync`:
   the migration runs as a preflight step before the rendered objects apply.
   No reverse-engineering.
3. **It gets set up for you.** `cub installer setup --install-prerequisites`
   runs the preflight as part of apply. If the route needs a decision (e.g. the
   migration needs DB creds → *per-target*), the catalog names that one choice
   and offers a default; otherwise it just runs.
4. **The receipt proves it.** The disposition flips `routed → observed` with a
   receipt: it ran, here is the result, here is when, here is the freshness.

Contrast with Helm: it ran the hook, and you do not know what it did or whether
it worked unless you go digging. Same convenience, plus the truth.
...
One apply flow resolves all three. The user makes two named choices (who owns
CRDs; which StorageClass), the password is handled and disclosed, and every
decision has a receipt. The point: **hooks were not special.** Every hidden
behavior is read and resolved through the same four-state, named-route model.
...
1. **Vocabulary — resolved by [#684](https://github.com/confighub/helm-expt/pull/684).**
   *observed / routed / per-target / refused* is now the published user-facing
   ...
   engineering list) is **Appendix C, note 3**.
...
**Dispositions:** observed (route + live receipt), routed (route wired, evidence
pending), per-target (catalog names the decision; may recommend, never
auto-picks), refused (not run automatically; reason given).
...
`observed`/`routed` routes, and you get a running thing in one command.
