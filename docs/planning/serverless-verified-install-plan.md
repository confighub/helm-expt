# cub install: the serverless model for helm-expt
...
In all cases the target experience is that `helm install redis` (or any chart) is simulated by `cub install redis`, against an appropriate Kubernetes cluster. Restricting the demo set (no hooks, etc.) is acceptable. The main thing is that a wide range of default charts just work, and build trust.
...
Most of the model exists in the current tree. Path 1 of `try-now.md` is shape (a) minus one verb: `cub installer setup --pull packages/bitnami/redis/25.5.3 --base default --non-interactive --namespace redis` pulls the package, picks a named base variant, and renders locally; `npm run redis:verify-install:render` proves the rendered objects match the catalog acceptance contract. No login anywhere in that path. Login first appears in Path 2 (upload to ConfigHub Units), exactly where the capture-versus-graph line says it should.
...
1. **One verb.** Today the path is setup, then an npm verify, then a harness lane for apply, then cub-scout. `cub install redis` must be one command that does resolve, pull, verify, collect facts, apply, and record. Every stage exists; nothing composes them. Plumbing, not invention.
...
3. **Pick the variant.** installer.yaml already declares named bases with a default. `cub install redis` takes the default; `--variant ha` takes another. No values file, no render: a menu of pre-rendered, pre-scanned variants.
...
7. **Operate.** `cub list` reads in-cluster receipts. `cub diff` compares live against the recorded inventory. `cub upgrade` pulls the next pinned revision and shows the object-level diff before touching anything. `cub rollback` re-applies the previous inventory. `cub uninstall` deletes by inventory.

### Resolving shapes (a), (b), (c)

(a) is the product. (b) demotes to an optional flag: `cub install --record-to git@...` commits the artifact reference and receipt to a repo for teams wanting shared history, and the same artifact hands to Argo or Flux unchanged; making git the default would break the speed bar persona 1 lives or dies on. (c) shrinks to its anonymous-read core: the static index needs no server at all; an anonymous ConfigHub catalog UI is a marketing surface, not a dependency.
...
**Check after.** Where this beats Helm most visibly. `cub diff` shows object-level drift in one command, where Helm needs a plugin and two renders. `cub upgrade` shows the exact object diff before applying, the view Helm cannot produce. `cub rollback` returns to the precise prior inventory. The observation receipt proves the thing is alive.
...
$ cub install redis
  ...
  variant    default        (also: ha, no-crds, reuse-existing-secret)
  ...
  recorded   receipt -> cluster (cub list to view)
...
```
cub diff redis        # what changed vs. what I installed, object by object
cub upgrade redis     # shows the exact diff of the new version BEFORE applying
cub rollback redis    # returns to the exact prior state, not a guess
```
...
**Put the wall in the product, not the pricing page.** The serverless tool detects its own limits and says so when they bind. The sharpest instance is the second install: the moment someone runs `cub install redis` into a second namespace or cluster, the tool knows (the in-cluster records make it knowable) and says: "You now have 2 unrelated copies of bitnami/redis. Nothing connects them. Sign in to make them variants of one base: change the base once, both update, overrides protected." Same pattern at first upgrade ("this chart moved from 25.5.3 to 26.0.0 upstream; signed-in users got the diff, the override check, and one-click propagation across environments") and at first drift detection ("drift found; locally you can revert it; signed in you'd know who made it and whether it was authorized").
...
The user keeps typing `cub install redis`. The verb, the flags, and the transcript shape are identical before and after login. What changes is not the command but three things underneath it.
...
$ cub install redis
  ...
  variant    default        (org-approved: default, ha)
...
- One verb, two substrates. `cub install` inspects the auth context: no context, local mode; authenticated context, graph mode. `--local` forces serverless behavior even when logged in, for the user who wants it.
