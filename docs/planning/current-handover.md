...
| Live parity rerun queue | 2 active non-pass rows in the two-cluster parity lane: 0 watch, 2 blocked, 0 semantic parity defects. | Choosing the next exact row that needs an operating-policy or target-fit decision before rerun. |
...
broader live parity rerun queue: 0 watch, 2 blocked, 0 semantic parity defects in the active two-cluster queue
...
`NGINX-customer-acme-prod`, `MetricsServer-prod-us-east`,
...
and cleanup for a small web chart, a cluster-service chart, and a server-only
observability chart across production, staging, and customer-derived variants.
A blocked target-bound receipt also exists for
