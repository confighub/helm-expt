...
- **"Vendor funnel."** "Free catalog is bait for lock-in." Defense: the serverless tier is complete at N=1 by design, the artifacts are standard KRM YAML applied by standard means, and the exit is `kubectl get -o yaml`. Status: HELD if the never-cripple-local rule from the serverless doc holds.
