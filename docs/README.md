...
| [what-config-workshop-is.md](./user/what-config-workshop-is.md) | The four-part definition with a verb per part: a catalog of tested configuration published as OCI images you can pull and run anywhere, uploaded into ConfigHub as base variants and recorded; a cub plugin for checking, certifying, and publishing from the command line; platforms and stacks on demand for people and their assistants, checked before anything runs; and verified, certified, signed as the proof you can make yourself. Ends with what it does not cover yet. |
