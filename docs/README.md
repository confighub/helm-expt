...
| [what-config-workshop-is.md](./user/what-config-workshop-is.md) | Config Workshop in four parts, each described as what it is: a catalog that standardizes many upstream formats into one OCI form, with every image uploaded into ConfigHub as a base variant and recorded; the workshop cub plugin over those images; platforms and stacks on demand for people and their assistants; and verified, certified, signed at the artifact level. Each part ends with what you can do and what problem it solves, and the doc ends with what is not covered yet. |
