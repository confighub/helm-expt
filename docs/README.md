...
| [cub-noun-vocabulary.md](./planning/cub-noun-vocabulary.md) | The cub noun and verb table, aligned to the get-started tutorial. A config is checked and deployed for free, then installed into ConfigHub, released to go live, and promoted across environments. A stack adds certify and sandbox like eks-inference. install is where public config chains into your private org, and it is the account line. deploy consumes an existing OCI, while release produces ConfigHub's governed one. |
