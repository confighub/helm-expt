...
| [cub-noun-vocabulary.md](./planning/cub-noun-vocabulary.md) | The cub noun and verb table mapped onto the Config Workshop journey: config, app, stack, platform, and server, each with the verbs that cross the entry-to-mid seam. Installation is the through-line, anonymous install is free (Option A), the account line is at keep, cub installer is the engine underneath, and cub check retires into cub config sandbox. |
