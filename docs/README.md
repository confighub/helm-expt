...
| [image-registry-migration.md](./user/image-registry-migration.md) | Repoint a chart's image registry across a fleet, run live on 2026-07-04: rewrite the registry on the base with the digest intact, promote to three environments, leave one on the old registry, and prove where it landed from revision history and data reads. The migration a catalog change of hands forces, done as recorded config. |
