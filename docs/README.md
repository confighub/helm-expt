...
| [credential-own-goal-scoping.md](./planning/credential-own-goal-scoping.md) | Read-only scoping of the credential own-goal: the default `generated-passwords` base on postgresql/mysql/mongodb/rabbitmq/grafana ships a hardcoded, identical-every-install password placeholder (no regeneration) while the site sells "catch the baked credential"; per-chart table plus rename / default-to-existing-secret / real-generation fix options. |
