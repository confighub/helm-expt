...
| [free-path-pitch.md](./planning/free-path-pitch.md) | The free-path value proposition: lead with a "look before you install" pre-flight check (parity demoted to quiet assurance), the ship-today copy with what backs each line, the north-star pitch, and the three builds (misconfig scan, CRD-skew upgrade check, grounded AI explainer) that make the full pitch true. |
