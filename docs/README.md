...
| [Sveltos Kyverno fleet example](./demo/sveltos/kyverno-fleet.md) | Live one-cluster example: ConfigHub checks and approves a ClusterProfile, a portable OCI and Argo CD deliver it, and Sveltos installs Kyverno and restores drift on the matching workload cluster. |
...
| [config-catalog-doctrine.md](./reference/config-catalog-doctrine.md) | Maintained doctrine for the anonymous-to-managed boundary, source-neutral base variants, four OCI package roles, fleet delivery, apply policy, AI use, and keeping the human and machine views aligned. |
