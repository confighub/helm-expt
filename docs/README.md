...
| [cub-noun-vocabulary.md](./planning/cub-noun-vocabulary.md) | The cub noun and verb table mapped onto the Config Workshop journey. A config is checked, installed, kept, and promoted, a stack adds certify and sandbox like eks-inference, and a governed stack becomes a platform. Anonymous install is free with the account line at keep. Install consumes an existing OCI, while keep then promote makes ConfigHub produce a governed one. cub check becomes cub config check, and cub installer is the engine under every install. |
