...
1. Choose platform components and wiring through Kubara catalogs, `config.yaml`, and ordinary values overlays.
2. Run Kubara to generate the complete platform, add-ons, ApplicationSets, overrides, and cluster wiring.
3. Prepare, scan, commit, and push the complete portable hand-off at one exact Git revision with dependency locks, checksums, effective renders, and wiring facts.
4. Import that exact revision deterministically and publish component/config OCI packages plus a digest-bound platform index, while keeping target facts and Secret values external.
5. Load the result into the organization explicitly selected by the user, reconcile visible component definitions, target instances, lineage, wiring, and delivery Applications, then require a zero-action second run and zero-orphan receipt.
6. Add, promote, approve, roll back, and observe applications through ConfigHub while Argo CD remains the cluster reconciler.

Start with the [buyer overview](../../docs/demo/kubara/index.md) and [six-step adoption tutorial](../../docs/demo/kubara/adoption.md). Use [the complete mini-IDP runbook](../../docs/demo/kubara/single-platform.md), [the importer reference](../../examples/kubara/git-import/README.md), [the ordinary Kubara example](../../examples/kubara/current-platform/README.md), [historical v0.12 evidence](../../docs/demo/kubara/local-platform.md), or [customized overlays](../../docs/corpus/kubara-customized-overlays.md) when you need deeper detail.
