...
Current limit: One continuous Redis proof now imports chart 25.5.3, records a post-render replica change, reconciles chart 27.0.0 without losing that change, returns mutation previews, promotes through development and staging, and checks the same OCI digest on two Argo CD clusters. It closes a rollback ChangeSet before publishing and verifies the rollback on both clusters. The proof is a guarded script and receipt, not a finished ConfigHub App interface. The portable OCI used a temporary local registry, and the live observations were recorded locally rather than submitted to ConfigHub observation storage.
