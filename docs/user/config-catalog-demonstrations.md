...
This choice is based on what the configuration controls, not whether it started as Helm, AICR, `cub installer`, Kubara, Sveltos, or YAML. On 2026-08-23, the live `helm-catalog` org had 35 Spaces on the 7 common checks and 13 Spaces on those checks plus approval (5 production and 8 system configuration). The receipt includes every maintained starting format: helm 3, aicr 6, cub-installer 31, kubara 1, sveltos 1, timoni 2, rendered-config 4.
