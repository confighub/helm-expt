...
| --- | --- | --- |
| Selected live Helm-vs-ConfigHub comparison | 15 pass, 5 watch, 0 blocked | The selected top-20 rows compare regular Helm against ConfigHub delivery paths. |
| Two-cluster kind parity for all top-20 bases | 25 pass, 3 watch, 12 blocked, 0 semantic parity defects | Regular Helm runs in one vanilla kind cluster and `cub installer` output runs in another. |
