...
| --- | --- | --- |
| Selected live Helm-vs-ConfigHub comparison | 79 pass, 0 watch, 2 target-fit blocked | Selected top-20 and nearby rows compare regular Helm against ConfigHub direct apply and ConfigHub OCI/Argo delivery paths. The current blocked rows are Consul secure mesh and Logstash HA on a one-node proof target; those bases require at least three schedulable nodes. |
...
The current selected rerun queue has two target-fit rows and no semantic parity
