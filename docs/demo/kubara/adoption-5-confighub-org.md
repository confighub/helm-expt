...
- the exact reviewed request from Step 4;
- the same clean detached Git checkout;
- the compiled output and passing `oci-publication-receipt.json`;
- the completed external, secret-free target-fact attestation;
- an authenticated `cub` version 0.2.11 or later;
- the request-pinned ConfigHub context and organization;
...
Compare the displayed organization name, external organization ID, server URL,
and context to `/controlled/import/reviewed-request.yaml`. The importer also
rechecks the internal organization entity and these coordinates before every
mutation.

## 5.1 Review the exact materialization plan

Read, do not infer, the ordered plan produced in Step 4:

```text
/controlled/import/revision-1/import-plan.json
```
...
`delivery.workloadApplications`; rerun `--inspect-destination`, compile, and
package if those declarations change.

## 5.2 Apply the exact revision once
...
```text
/controlled/import/revision-1/apply-receipt.json
```

That first receipt records a converged attempt, not accepted idempotence.

## 5.3 Apply the identical inputs immediately again
...
## 5.4 Verify cluster convergence separately
...
This repository also has a stricter, canned four-cluster mini-IDP reconciler
for the ConfigHub `Kubara` example organization. It is not a generic command
for another adopter's organization.
...
At the time this chapter was written, those **source-current live receipts are
absent and still need to be refreshed**. The scripts and desired state are
implemented, but the website must not present the current organization,
idempotence, convergence, or zero-orphan claim as proved until both files
exist, report `pass`, and their receipt-verification commands succeed.
...
`/controlled/import/revision-1/apply-receipt.json` and require the exact passing
second-run fields above. Then retain the separate Argo/Kubernetes observations.
...
Before the first apply, stopping is always safe: Step 4 has not changed
ConfigHub. After an interrupted first apply, keep all exact inputs and rerun the
same command; the importer is designed for bounded additive recovery. Do not
switch organization, alter the request, or start a second writer during that
recovery.
