# The h100-eks-ubuntu-inference-dynamo overlay, mirrored as a catalog entry

UNOFFICIAL/EXPERIMENTAL. This entry belongs to
[the AICR catalog overview](./index.md). It was produced by
`scripts/generate-aicr-from-overlay.mjs h100-eks-ubuntu-inference-dynamo`, the generator built
...
Criteria `--service eks --accelerator h100 --os ubuntu --intent inference --platform dynamo` resolves eight overlays into 18 components,
...
```bash
aicr recipe --service eks --accelerator h100 --os ubuntu --intent inference --platform dynamo --output recipe.yaml
...
The [generation receipt](../../../examples/aicr/h100-eks-ubuntu-inference-dynamo/generation-receipt.yaml)
...
```bash
node scripts/generate-aicr-digest-index.mjs --verify --example h100-eks-ubuntu-inference-dynamo
```

The [digest index](../../../examples/aicr/h100-eks-ubuntu-inference-dynamo/digest-index/README.md)
