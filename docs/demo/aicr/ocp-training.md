# The ocp-training overlay, mirrored as a catalog entry

UNOFFICIAL/EXPERIMENTAL. This entry belongs to
[the AICR catalog overview](./index.md). It was produced by
`scripts/generate-aicr-from-overlay.mjs ocp-training`, the generator built
...
Criteria `--service ocp --intent training` resolves four overlays into 10 components,
...
```bash
aicr recipe --service ocp --intent training --output recipe.yaml
...
The [generation receipt](../../../examples/aicr/ocp-training/generation-receipt.yaml)
...
```bash
node scripts/generate-aicr-digest-index.mjs --verify --example ocp-training
```

The [digest index](../../../examples/aicr/ocp-training/digest-index/README.md)
