# The gb200-oke-ubuntu-training-kubeflow overlay, mirrored as a catalog entry

UNOFFICIAL/EXPERIMENTAL. This entry belongs to
[the AICR catalog overview](./index.md). It was produced by
`scripts/generate-aicr-from-overlay.mjs gb200-oke-ubuntu-training-kubeflow`, the generator built
...
Criteria `--service oke --accelerator gb200 --os ubuntu --intent training --platform kubeflow` resolves eight overlays into 13 components,
...
```bash
aicr recipe --service oke --accelerator gb200 --os ubuntu --intent training --platform kubeflow --output recipe.yaml
...
The [generation receipt](../../../examples/aicr/gb200-oke-ubuntu-training-kubeflow/generation-receipt.yaml)
...
```bash
node scripts/generate-aicr-digest-index.mjs --verify --example gb200-oke-ubuntu-training-kubeflow
```

The [digest index](../../../examples/aicr/gb200-oke-ubuntu-training-kubeflow/digest-index/README.md)
