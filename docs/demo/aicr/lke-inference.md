# The lke-inference overlay, mirrored as a catalog entry

UNOFFICIAL/EXPERIMENTAL. This entry belongs to
[the AICR catalog overview](./index.md). It was produced by
`scripts/generate-aicr-from-overlay.mjs lke-inference`, the generator built
...
Criteria `--service lke --intent inference` resolves four overlays into 13 components,
...
```bash
aicr recipe --service lke --intent inference --output recipe.yaml
...
The [generation receipt](../../../examples/aicr/lke-inference/generation-receipt.yaml)
...
```bash
node scripts/generate-aicr-digest-index.mjs --verify --example lke-inference
```

The [digest index](../../../examples/aicr/lke-inference/digest-index/README.md)
