# The h100-gke-cos-inference overlay, mirrored as a catalog entry

UNOFFICIAL/EXPERIMENTAL. This entry belongs to
[the AICR catalog overview](./index.md). It was produced by
`scripts/generate-aicr-from-overlay.mjs h100-gke-cos-inference`, the generator built
...
Criteria `--service gke --accelerator h100 --os cos --intent inference` resolves six overlays into 16 components,
...
```bash
aicr recipe --service gke --accelerator h100 --os cos --intent inference --output recipe.yaml
...
The [generation receipt](../../../examples/aicr/h100-gke-cos-inference/generation-receipt.yaml)
...
```bash
node scripts/generate-aicr-digest-index.mjs --verify --example h100-gke-cos-inference
```

The [digest index](../../../examples/aicr/h100-gke-cos-inference/digest-index/README.md)
