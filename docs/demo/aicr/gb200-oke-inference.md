# The gb200-oke-inference overlay, mirrored as a catalog entry

UNOFFICIAL/EXPERIMENTAL. This entry belongs to
[the AICR catalog overview](./index.md). It was produced by
`scripts/generate-aicr-from-overlay.mjs gb200-oke-inference`, the generator built
...
Criteria `--service oke --accelerator gb200 --os ol --intent inference` resolves six overlays into 13 components,
...
```bash
aicr recipe --service oke --accelerator gb200 --os ol --intent inference --output recipe.yaml
...
The [generation receipt](../../../examples/aicr/gb200-oke-inference/generation-receipt.yaml)
...
```bash
node scripts/generate-aicr-digest-index.mjs --verify --example gb200-oke-inference
```

The [digest index](../../../examples/aicr/gb200-oke-inference/digest-index/README.md)
