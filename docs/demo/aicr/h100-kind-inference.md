# The h100-kind-inference overlay, mirrored as a catalog entry

UNOFFICIAL/EXPERIMENTAL. This entry belongs to
[the AICR catalog overview](./index.md). It was produced by
`scripts/generate-aicr-from-overlay.mjs h100-kind-inference`, the generator built
...
Criteria `--service kind --accelerator h100 --intent inference` resolves six overlays into 14 components,
...
```bash
aicr recipe --service kind --accelerator h100 --intent inference --output recipe.yaml
...
The [generation receipt](../../../examples/aicr/h100-kind-inference/generation-receipt.yaml)
...
```bash
node scripts/generate-aicr-digest-index.mjs --verify --example h100-kind-inference
```

The [digest index](../../../examples/aicr/h100-kind-inference/digest-index/README.md)
