# The h100-bcm-training overlay, mirrored as a catalog entry

UNOFFICIAL/EXPERIMENTAL. This entry belongs to
[the AICR catalog overview](./index.md). It was produced by
`scripts/generate-aicr-from-overlay.mjs h100-bcm-training`, the generator built
...
Criteria `--service bcm --accelerator h100 --intent training` resolves six overlays into 12 components,
...
```bash
aicr recipe --service bcm --accelerator h100 --intent training --output recipe.yaml
...
The [generation receipt](../../../examples/aicr/h100-bcm-training/generation-receipt.yaml)
...
```bash
node scripts/generate-aicr-digest-index.mjs --verify --example h100-bcm-training
```

The [digest index](../../../examples/aicr/h100-bcm-training/digest-index/README.md)
