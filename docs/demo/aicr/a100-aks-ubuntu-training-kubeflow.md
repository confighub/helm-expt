# The a100-aks-ubuntu-training-kubeflow overlay, mirrored as a catalog entry
...
Criteria `--service aks --accelerator a100 --os ubuntu --intent training --platform kubeflow` resolves eight overlays into 14 components,
...
```bash
aicr recipe --service aks --accelerator a100 --os ubuntu --intent training --platform kubeflow --output recipe.yaml
...
The [generation receipt](../../../examples/aicr/a100-aks-ubuntu-training-kubeflow/generation-receipt.yaml)
...
```bash
node scripts/generate-aicr-digest-index.mjs --verify --example a100-aks-ubuntu-training-kubeflow
```

The [digest index](../../../examples/aicr/a100-aks-ubuntu-training-kubeflow/digest-index/README.md)
