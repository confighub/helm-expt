# The a100-oke-ubuntu-training-kubeflow overlay, mirrored as a catalog entry

UNOFFICIAL/EXPERIMENTAL. This entry belongs to
[the AICR catalog overview](./index.md). It was produced by
`scripts/generate-aicr-from-overlay.mjs a100-oke-ubuntu-training-kubeflow`, the generator built
...
Criteria `--service oke --accelerator a100 --os ubuntu --intent training --platform kubeflow` resolves eight overlays into 12 components,
and the Argo CD bundle renders into 14 Applications (one platform root plus one
...
```bash
aicr recipe --service oke --accelerator a100 --os ubuntu --intent training --platform kubeflow --output recipe.yaml
...
The [generation receipt](../../../examples/aicr/a100-oke-ubuntu-training-kubeflow/generation-receipt.yaml)
...
```bash
node scripts/generate-aicr-digest-index.mjs --verify --example a100-oke-ubuntu-training-kubeflow
```

The [digest index](../../../examples/aicr/a100-oke-ubuntu-training-kubeflow/digest-index/README.md)
pins the upstream source, the recipe criteria, and all 14 Applications
