# prometheus-community/prometheus 29.8.0 Proof

This is the promoted proof slice for the Prometheus public Helm chart.

Variants:

- `default`: chart defaults with server, Alertmanager, exporters, PVC, and RBAC; 23 Helm objects, 24 cub install objects including Namespace.
- `server-only-ephemeral`: only the Prometheus server is rendered and persistence is disabled; 6 Helm objects, 7 cub install objects including Namespace.
