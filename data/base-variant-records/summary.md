...
There are **259 records**: 4 aicr, 1 configuration-oci, 1 cub-installer, 248 helm, 1 kubara, 1 kubernetes-yaml, 1 sveltos, 2 timoni. Their current statuses are 249 available, 10 partial.
...
- Flattening verdicts: 3 born-flattened, 75 flatten-with-routes, 14 not-assessed, 70 safe-to-flatten, 97 unsafe-to-flatten.
- Lifecycle requirements: 156 gap, 12 not-required, 91 recorded.
- Portable route intents: 156 gap, 12 not-required, 16 recorded, 75 required-at-destination.
- Variant-and-destination route resolution: 86 awaits-variant-and-target, 2 blocked, 156 gap, 12 not-required, 3 resolved-for-recorded-targets.
- Field ownership: 2 declared, 257 partly-declared.
- Inspection evidence: 259 completed; results: 259 available.
- Materialization evidence: 259 completed; results: 259 pass.
- Destination evidence: 2 blocked, 3 completed, 98 not-run, 156 pending; results: 2 blocked, 98 not-run, 3 pass, 156 pending.
- Post-deployment evidence: 3 completed, 256 not-run; results: 256 not-run, 2 pass, 1 watch.

## Model and Catalog alignment

All **259/259 records** use the same source-neutral structure. Every row identifies its source selection and provider, source record, retained base-revision digest role, exact-object digest role, materialization result, flattening status, lifecycle requirements, route intent, target-resolution status, field ownership, delivery evidence, and current limits. Every row also answers the same four questions separately: what the source contains, what it produces, whether a named destination can accept it, and what happened after deployment.

The evidence is not complete for every row:

| Area | Current Catalog coverage | Which is ahead? |
| --- | ---: | --- |
| Exact source and object records | 259/259 | Catalog and model are aligned. |
...
| Flattening verdicts | 245/259 | The model is ahead of the unassessed rows. |
| Destination-specific route resolutions | 3/259 | The model is ahead. The resolved records cover one Helm configuration through direct apply, Argo CD, and Flux. |
| Fully declared field ownership | 2/259 | The model is ahead. Most Helm rows currently separate source values from post-render changes without field-by-field ownership. |
