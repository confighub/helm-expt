...
There are **261 records**: 4 aicr, 1 configuration-oci, 1 cub-installer, 250 helm, 1 kubara, 1 kubernetes-yaml, 1 sveltos, 2 timoni. Their current statuses are 251 available, 10 partial.
...
- Flattening verdicts: 3 born-flattened, 75 flatten-with-routes, 12 not-assessed, 72 safe-to-flatten, 99 unsafe-to-flatten.
- Lifecycle requirements: 158 gap, 12 not-required, 91 recorded.
- Portable route intents: 158 gap, 12 not-required, 16 recorded, 75 required-at-destination.
- Variant-and-destination route resolution: 86 awaits-variant-and-target, 2 blocked, 158 gap, 12 not-required, 3 resolved-for-recorded-targets.
- Field ownership: 2 declared, 259 partly-declared.
- Inspection evidence: 261 completed; results: 261 available.
- Materialization evidence: 261 completed; results: 261 pass.
- Destination evidence: 2 blocked, 3 completed, 98 not-run, 158 pending; results: 2 blocked, 98 not-run, 3 pass, 158 pending.
- Post-deployment evidence: 3 completed, 258 not-run; results: 258 not-run, 2 pass, 1 watch.

## Model and Catalog alignment

All **261/261 records** use the same source-neutral structure. Every row identifies its source selection and provider, source record, retained base-revision digest role, exact-object digest role, materialization result, flattening status, lifecycle requirements, route intent, target-resolution status, field ownership, delivery evidence, and current limits. Every row also answers the same four questions separately: what the source contains, what it produces, whether a named destination can accept it, and what happened after deployment.

The evidence is not complete for every row:

| Area | Current Catalog coverage | Which is ahead? |
| --- | ---: | --- |
| Exact source and object records | 261/261 | Catalog and model are aligned. |
...
| Flattening verdicts | 249/261 | The model is ahead of the unassessed rows. |
| Destination-specific route resolutions | 3/261 | The model is ahead. The resolved records cover one Helm configuration through direct apply, Argo CD, and Flux. |
| Fully declared field ownership | 2/261 | The model is ahead. Most Helm rows currently separate source values from post-render changes without field-by-field ownership. |
