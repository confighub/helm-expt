...
There are **253 records**: 3 aicr, 1 configuration-oci, 1 cub-installer, 245 helm, 1 kubara, 1 kubernetes-yaml, 1 sveltos. Their current statuses are 246 available, 7 partial.
...
- Flattening verdicts: 3 born-flattened, 33 flatten-with-routes, 167 not-assessed, 41 safe-to-flatten, 9 unsafe-to-flatten.
- Lifecycle requirements: 167 gap, 86 recorded.
- Portable route intents: 167 gap, 13 recorded, 73 required-at-destination.
- Variant-and-destination route resolution: 82 awaits-variant-and-target, 1 blocked, 167 gap, 3 resolved-for-recorded-targets.
- Field ownership: 1 declared, 252 partly-declared.

## Model and Catalog alignment

All **253/253 records** use the same source-neutral structure. Every row identifies its source record, base-revision digest role, exact-object digest role, materialization result, flattening status, lifecycle requirements, route intent, target-resolution status, field ownership, delivery evidence, and current limits.

The evidence is not complete for every row:

| Area | Current Catalog coverage | Which is ahead? |
| --- | ---: | --- |
| Exact source and object records | 253/253 | Catalog and model are aligned. |
...
| Flattening verdicts | 86/253 | The model is ahead of the unassessed rows. |
| Destination-specific route resolutions | 3/253 | The model is ahead. The resolved records cover one Helm configuration through direct apply, Argo CD, and Flux. |
| Fully declared field ownership | 1/253 | The model is ahead. Most Helm rows currently separate source values from post-render changes without field-by-field ownership. |
