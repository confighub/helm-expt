...
There are **256 records**: 4 aicr, 1 configuration-oci, 1 cub-installer, 245 helm, 1 kubara, 1 kubernetes-yaml, 1 sveltos, 2 timoni. Their current statuses are 246 available, 10 partial.
...
- Flattening verdicts: 3 born-flattened, 75 flatten-with-routes, 11 not-assessed, 70 safe-to-flatten, 97 unsafe-to-flatten.
- Lifecycle requirements: 155 gap, 12 not-required, 89 recorded.
- Portable route intents: 155 gap, 12 not-required, 16 recorded, 73 required-at-destination.
- Variant-and-destination route resolution: 84 awaits-variant-and-target, 2 blocked, 155 gap, 12 not-required, 3 resolved-for-recorded-targets.
- Field ownership: 2 declared, 254 partly-declared.
- Inspection evidence: 256 completed; results: 256 available.
- Materialization evidence: 256 completed; results: 256 pass.
- Destination evidence: 2 blocked, 3 completed, 96 not-run, 155 pending; results: 2 blocked, 96 not-run, 3 pass, 155 pending.
- Post-deployment evidence: 3 completed, 253 not-run; results: 253 not-run, 2 pass, 1 watch.

## Model and Catalog alignment

All **256/256 records** use the same source-neutral structure. Every row identifies its source selection and provider, source record, retained base-revision digest role, exact-object digest role, materialization result, flattening status, lifecycle requirements, route intent, target-resolution status, field ownership, delivery evidence, and current limits. Every row also answers the same four questions separately: what the source contains, what it produces, whether a named destination can accept it, and what happened after deployment.

The evidence is not complete for every row:

| Area | Current Catalog coverage | Which is ahead? |
| --- | ---: | --- |
| Exact source and object records | 256/256 | Catalog and model are aligned. |
...
| Flattening verdicts | 245/256 | The model is ahead of the unassessed rows. |
| Destination-specific route resolutions | 3/256 | The model is ahead. The resolved records cover one Helm configuration through direct apply, Argo CD, and Flux. |
| Fully declared field ownership | 2/256 | The model is ahead. Most Helm rows currently separate source values from post-render changes without field-by-field ownership. |
