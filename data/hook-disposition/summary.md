...
| routed | 4 | A route is selected and worked evidence exists (rendered hook resources; live attempted or explicitly blocked). |
| per-target | 1 | The right route depends on the target cluster class; no single route claim is honest. |
| recipe-needed | 3 | A route plan exists but the chart has no recipe, so no maintained lifecycle work can attach yet. |
...
| 32 | bitnami/minio@17.0.21 | 1 (post-install, post-upgrade) | chart-own | recipe-needed | none |
...
- bitnami/minio's worked evidence is owned by the active rehearsal lane and is deliberately not duplicated here.
