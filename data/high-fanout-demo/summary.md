...
| `no-crds` | install the stack without creating CRDs | 114 | 0 | 2 | 50 | render `pass`; two-cluster kind `pass`; strict ConfigHub OCI/Argo `pass`; production `separate-support-decision-needed` |
...
| `no-crds` | record a target-scoped production support decision that reuses the proven staged-CRD and admission-Secret OCI path |
