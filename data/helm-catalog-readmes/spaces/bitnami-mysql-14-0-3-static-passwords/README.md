...
| --- | --- | --- |
| Helm values | [The values profile](https://github.com/confighub/helm-expt/blob/main/recipes/bitnami/mysql/14.0.3/effective-values.yaml) defines the `static-passwords` base together with release `mysql` and namespace `mysql`. | Change the values and create or update a base preset when Helm needs to produce different objects. |
...
| Helm values profile | [recipes/bitnami/mysql/14.0.3/effective-values.yaml](https://github.com/confighub/helm-expt/blob/main/recipes/bitnami/mysql/14.0.3/effective-values.yaml) |
