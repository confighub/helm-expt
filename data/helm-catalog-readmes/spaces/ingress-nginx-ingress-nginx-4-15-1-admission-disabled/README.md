...
For this Space, the answer is the `admission-disabled` preset. ingress-nginx admission webhook disabled variant rendered from ingress-nginx/ingress-nginx@4.15.1.
...
| --- | --- | --- |
| Helm values | [The values profile](https://github.com/confighub/helm-expt/blob/main/recipes/ingress-nginx/ingress-nginx/4.15.1/effective-values-admission-disabled.yaml) defines the `admission-disabled` base together with release `ingress-nginx` and namespace `ingress-nginx`. | Change the values and create or update a base preset when Helm needs to produce different objects. |
...
| Helm values profile | [recipes/ingress-nginx/ingress-nginx/4.15.1/effective-values-admission-disabled.yaml](https://github.com/confighub/helm-expt/blob/main/recipes/ingress-nginx/ingress-nginx/4.15.1/effective-values-admission-disabled.yaml) |
