...
This Space exists to answer one practical question: what is a safe, repeatable way to run `argo-cd/argo-cd@9.5.15` for this operating choice?
...
- Preset: `no-crds`.
- Kubernetes YAML: 46 object(s), mainly ConfigMap x8, Deployment x6, Role x6, RoleBinding x6, ServiceAccount x6, Service x5, ClusterRole x3, ClusterRoleBinding x3.
- Needs before install: 3 CRDs, 1 Secret.
...
| --- | --- | --- |
| Helm values | [The values profile](https://github.com/confighub/helm-expt/blob/main/recipes/argo-cd/argo-cd/9.5.15/effective-values-no-crds.yaml) defines the `no-crds` base together with release `argo-cd` and namespace `argocd`. | Change the values and create or update a base preset when Helm needs to produce different objects. |
...
| Install work | 3 CRDs, 1 Secret; no separate hook or setup route is recorded. | Follow the prerequisite and route records. Secrets, CRDs, and setup jobs are not hidden as values or ConfigHub edits. |
...
| Item | Link |
| --- | --- |
| Catalog chart page | [argo-cd/argo-cd@9.5.15](https://confighub.github.io/helm-expt/site/charts/argo-cd-argo-cd-9-5-15.html) |
| Helm values profile | [recipes/argo-cd/argo-cd/9.5.15/effective-values-no-crds.yaml](https://github.com/confighub/helm-expt/blob/main/recipes/argo-cd/argo-cd/9.5.15/effective-values-no-crds.yaml) |
...
| Detailed guide | [data/confighub-example-guides/spaces/argo-cd-argo-cd-9-5-15-no-crds/README.md](https://github.com/confighub/helm-expt/blob/main/data/confighub-example-guides/spaces/argo-cd-argo-cd-9-5-15-no-crds/README.md) |
| No-account script | [try.sh](https://confighub.github.io/helm-expt/site/sh/argo-cd-argo-cd-9-5-15/no-crds/try.sh) |
| ConfigHub upload script | [confighub.sh](https://confighub.github.io/helm-expt/site/sh/argo-cd-argo-cd-9-5-15/no-crds/confighub.sh) |
