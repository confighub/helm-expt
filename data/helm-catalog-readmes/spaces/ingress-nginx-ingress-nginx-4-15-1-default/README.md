...
- Preset: `default`.
- Kubernetes YAML: 11 object(s), mainly Service x2, ClusterRole x1, ClusterRoleBinding x1, ConfigMap x1, Deployment x1, IngressClass x1, Role x1, RoleBinding x1.
...
| --- | --- | --- |
| Helm values | [The values profile](https://github.com/confighub/helm-expt/blob/main/recipes/ingress-nginx/ingress-nginx/4.15.1/effective-values.yaml) defines the `default` base together with release `ingress-nginx` and namespace `ingress-nginx`. | Change the values and create or update a base preset when Helm needs to produce different objects. |
...
| Helm values profile | [recipes/ingress-nginx/ingress-nginx/4.15.1/effective-values.yaml](https://github.com/confighub/helm-expt/blob/main/recipes/ingress-nginx/ingress-nginx/4.15.1/effective-values.yaml) |
