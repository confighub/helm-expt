...
For this Space, the answer is the `ha-raft-ui` preset. Use this when you want the reviewed high-availability shape instead of the simplest default.
...
- Preset: `ha-raft-ui`.
- Kubernetes YAML: 18 object(s), mainly Service x6, ClusterRoleBinding x2, ServiceAccount x2, ClusterRole x1, ConfigMap x1, Deployment x1, MutatingWebhookConfiguration x1, PodDisruptionBudget x1.
- Needs before install: 1 other item.
...
| --- | --- | --- |
| Helm values | [The values profile](https://github.com/confighub/helm-expt/blob/main/recipes/hashicorp/vault/0.32.0/effective-values-ha-raft-ui.yaml) defines the `ha-raft-ui` base together with release `vault` and namespace `vault`. | Change the values and create or update a base preset when Helm needs to produce different objects. |
...
| Install work | 1 other item; no separate hook or setup route is recorded. | Follow the prerequisite and route records. Secrets, CRDs, and setup jobs are not hidden as values or ConfigHub edits. |
...
| Helm values profile | [recipes/hashicorp/vault/0.32.0/effective-values-ha-raft-ui.yaml](https://github.com/confighub/helm-expt/blob/main/recipes/hashicorp/vault/0.32.0/effective-values-ha-raft-ui.yaml) |
