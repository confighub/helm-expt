...
This is the per-chart view of the adoption audit (docs/planning/helm-vs-cub-adoption-audit.md, PR #1016): for each of the 112 charts, which "worse-than / more-confusing-than plain Helm" rough edges apply, and the exact fix.

## Three caveats that apply to EVERY chart (cub-direct path)

These are the same for all 112 charts, so they are stated once here rather than repeated per chart. **Use a controller (Argo/Flux) and all three are handled for you**; on the bare cub-direct path the managed applier handles them.
...
**4/112 charts** still bake a shared placeholder password in their default setup; **47/112 charts** ship CRDs that need first-ordering on cub-direct. Full data for all 112 charts: [caveats.csv](./caveats.csv) · [summary.html](./summary.html).
...
### CRDs need first-ordering on cub-direct (#1015) — 47 charts
