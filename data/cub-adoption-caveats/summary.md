...
This is the per-chart view of the adoption audit (docs/planning/helm-vs-cub-adoption-audit.md, PR #1016): for each of the 113 charts, which "worse-than / more-confusing-than plain Helm" rough edges apply, and the exact fix.

## Three caveats that apply to EVERY chart (cub-direct path)

These are the same for all 113 charts, so they are stated once here rather than repeated per chart. **Use a controller (Argo/Flux) and all three are handled for you**; on the bare cub-direct path the managed applier handles them.
...
**4/113 charts** still bake a shared placeholder password in their default setup; **48/113 charts** ship CRDs that need first-ordering on cub-direct. Full data for all 113 charts: [caveats.csv](./caveats.csv) · [summary.html](./summary.html).

### Shared placeholder password (#1012) — 4 charts
...
### CRDs need first-ordering on cub-direct (#1015) — 48 charts
...
| aws-controllers-k8s/ec2-chart | `default` | 22 | apply CRDs first + wait, or use a controller (#1015/#1019) |
...
| karpenter/karpenter | `default` | 5 | apply CRDs first + wait, or use a controller (#1015/#1019) |
...
| mysql/mysql-operator | `default` | 5 | apply CRDs first + wait, or use a controller (#1015/#1019) |
...
| traefik/traefik | `default` | 25 | apply CRDs first + wait, or use a controller (#1015/#1019) |
