...
This is the per-chart view of the adoption audit (docs/planning/helm-vs-cub-adoption-audit.md, PR #1016): for each of the 108 charts, which "worse-than / more-confusing-than plain Helm" rough edges apply, and the exact fix.

## Three caveats that apply to EVERY chart (cub-direct path)

These are the same for all 108 charts, so they are stated once here rather than repeated per chart. **Use a controller (Argo/Flux) and all three are handled for you**; on the bare cub-direct path the managed applier handles them.
...
**2/108 charts** still bake a shared placeholder password in their default setup; **47/108 charts** ship CRDs that need first-ordering on cub-direct. Full data for all 108 charts: [caveats.csv](./caveats.csv) · [summary.html](./summary.html).
