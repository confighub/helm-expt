...
| `no-crds` | try-with-proof | render=pass; confighub=missing; local=missing; gitops=blocked; live-parity=missing; two-cluster=pass | required Secret argocd/argocd-redis keys auth; required CRD applications.argoproj.io; required CRD applicationsets.argoproj.io; required CRD appprojects.argoproj.io | `cub installer setup --pull packages/argo-cd/argo-cd/9.5.15 --base no-crds --work-dir <tmp> --non-interactive --namespace argocd` |
