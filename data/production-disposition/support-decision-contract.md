...
| Image digest resolution | 7 | Pin images by digest or record the explicit exception before claiming production OCI support.<br>`bitnami/mysql@14.0.3` (generated-passwords)<br>`bitnami/rabbitmq@16.0.14` (generated-passwords)<br>`grafana/grafana@10.5.15` (generated-passwords)<br>`grafana/loki@7.0.0` (single-binary-filesystem)<br>`grafana/tempo@1.24.4` (local-persistent)<br>and 2 more |
...
| Close open dispositions | 3 | Write or fix the missing disposition receipts before making a support decision.<br>`argo-cd/argo-cd@9.5.15` (default)<br>`external-secrets/external-secrets@2.5.0` (default)<br>`prometheus-community/kube-prometheus-stack@85.3.3` (default) |

| Chart | Candidate base | Base readiness | Decision state | Next action |
| --- | --- | --- | --- | --- |
| `argo-cd/argo-cd@9.5.15` | default | try-with-proof | close-dispositions-first | write or fix the receipt for target fact preflight |
...
| `grafana/tempo@1.24.4` | local-persistent | try-with-proof | resolve-images-before-production-oci | resolve image digests for each affected variant before production OCI support |
