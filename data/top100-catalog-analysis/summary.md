...
This is the generated proof-surface view for the 110 maintained
...
next-80 proof-grade entries: 90
...
hard gap for at least one recommended capability: 23
```
...
| Tier | Count | Meaning |
| --- | ---: | --- |
| Recipe/package proof exists | 110 | The chart has a maintained recipe and executable `cub installer` package with proof artifacts. |
...
| Proof-grade, not catalog-supported | 90 | The chart has deterministic proof artifacts but still needs user-shaped variants and promotion review. |
...
| Hard gap for a recommended capability | 23 | The core recipe may still work, but at least one useful capability is not yet enabled. See `data/chart-facts/summary.md`. |
...
works, but still needs user-shaped product review
  90 charts are proof-grade but not catalog-supported.

works only with a named limitation or user/operator help
  23 charts have at least one hard gap for a recommended extra capability.
```
...
| --- | ---: | --- |
| Can the repo prove a maintained recipe/package path exists? | 110 | Yes. Every top-100 entry has recipe/package proof artifacts. |
| Can a user try it through the public local-test catalog lane today? | 20 | Yes, for the top-20 declared local-test scope. |
| Does it still need catalog promotion review before support is claimed? | 90 | Yes. These entries are proof-grade, not catalog-supported. |
...
| Does it have a named hard gap for at least one recommended capability? | 23 | Yes. The core recipe may work, but one useful variant or capability is missing or blocked. |
