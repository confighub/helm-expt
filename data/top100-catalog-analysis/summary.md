...
retained newer candidate versions: 12
catalog-supported: 20
proof-grade: 85
multi-variant entries: 74
...
| Variant-rich | 74 | The chart has more than one base variant. |
| Default-only | 36 | The chart has a proof path, but not enough variants for common user choices. |
| No hard gap in chart-facts | 88 | Recommended capabilities are built, not applicable, or have a known path. |
...
| Does it already have more than one base variant? | 74 | Yes. These entries cover more than the default shape. |
