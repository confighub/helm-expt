...
```text
ready-to-run rows:   28
run blocks:          6  (G/P: 2, K: 4)
derived predictions: 18
...
**Goal:** K: run the 5 ready kind-parity row(s) with predicted gitops-runtime / target-prerequisite / remote-image (minio-operator/operator@7.1.1, rook-release/rook-ceph-cluster@v1.19.5, rook-release/rook-ceph@v1.19.5, bitnami/apache@11.4.29, bitnami/elasticsearch@22.1.6); each kind-parity command classifies the K cell. Confirm or reclassify from each receipt.
...
**Goal:** K: run the 5 ready kind-parity row(s) with predicted remote-image (bitnami/elasticsearch@22.1.6, bitnami/phpmyadmin@20.0.0, bitnami/spark@10.0.3, bitnami/zookeeper@13.8.7); each kind-parity command classifies the K cell. Confirm or reclassify from each receipt.
...
**Goal:** K: run the 5 ready kind-parity row(s) with predicted remote-image / target-runtime / unknown (bitnami/zookeeper@13.8.7, elastic/kibana@8.5.1, elastic/metricbeat@8.5.1, grafana/pyroscope@2.0.2, dex/dex@0.24.0); each kind-parity command classifies the K cell. Confirm or reclassify from each receipt.
