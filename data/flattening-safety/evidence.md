...
Every catalog entry with a recorded witness is scanned for the constructs that render-time flattening loses. This view reports what 133 packaged charts contain. It does not decide whether any of them may ship as flattened YAML: that is a flattening-safety verdict, which weighs values gating, the audited base, and the routes available to discharge each construct. 133 of these chart versions have a decided lane today; the rest read "not yet decided" rather than reading as safe by omission.

| construct | charts | share | what it costs a flattened bundle |
| --- | --- | --- | --- |
| capabilities | 96 | 72% | the wrong apiVersion for the target cluster |
...
| hooks | 48 | 36% | Hook Jobs never fire, or fire under a different hook dialect |
| generated-secrets | 40 | 30% | one credential draw frozen into a shared artifact |
| keep-policy | 31 | 23% | a reconciler prunes what Helm promised to keep |
...
| test-hooks | 25 | 19% | stray test resources shipped to a cluster |
