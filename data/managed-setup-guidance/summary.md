...
| bitnami | `generated-passwords` | yes | `existing-secret` | The default base (`generated-passwords`) bakes a password Secret (2 password key(s)) — a shared placeholder, identical for everyone. Use the `existing-secret` base and supply your own Secret, or replace the placeholder before prod. |

### The determinism tension (why cub can't just "generate a random password")

cub's deterministic render (#1011, a real win) and Helm's random-per-install password are mutually exclusive from one render. So the secret answer is reuse-existing-secret (supply/reference your own), not generate-random-in-render. The `generated-passwords` base name promises what determinism forbids and should be renamed.
