...
charts with live evidence on at least one variant: 69
...
| Expand live evidence | 31 | Select rows that only have render parity and add local, GitOps, or live Helm-vs-ConfigHub evidence. | The strongest evidence moves beyond render parity for the selected chart/base. | `gitlab/gitlab-runner@0.89.0`<br>`nfs-subdir-external-provisioner/nfs-subdir-external-provisioner@4.0.18`<br>`elastic/kibana@8.5.1`<br>`bitnami/elasticsearch@22.1.6`<br>`dex/dex@0.24.0` |
...
| `local-kubernetes-live` | 38 | Rendered objects were applied to Kubernetes and observed for at least one variant. |
| `render-parity` | 31 | Regular Helm and cub installer setup render-equivalent objects. |
