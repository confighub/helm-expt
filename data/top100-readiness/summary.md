...
| `local-kubernetes-live` | 40 | Rendered objects were applied to Kubernetes and observed for at least one variant. |
| `render-parity` | 15 | Regular Helm and cub installer setup render-equivalent objects. |
| `two-cluster-kind-parity` | 12 | Plain Helm and cub installer output reached equivalent live outcomes in separate vanilla kind clusters. |
