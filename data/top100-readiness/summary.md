...
charts with live evidence on at least one variant: 38
...
| Expand live evidence | 62 | Select rows that only have render parity and add local, GitOps, or live Helm-vs-ConfigHub evidence. | The strongest evidence moves beyond render parity for the selected chart/base. | `traefik/traefik@40.2.0`<br>`gitlab/gitlab-runner@0.89.0`<br>`runix/pgadmin4@1.62.0`<br>`nfs-subdir-external-provisioner/nfs-subdir-external-provisioner@4.0.18`<br>`prometheus-community/kube-state-metrics@7.4.0` |
...
| `render-parity` | 62 | Regular Helm and cub installer setup render-equivalent objects. |
