...
charts with live evidence on at least one variant: 51
...
| Expand live evidence | 49 | Select rows that only have render parity and add local, GitOps, or live Helm-vs-ConfigHub evidence. | The strongest evidence moves beyond render parity for the selected chart/base. | `traefik/traefik@40.2.0`<br>`gitlab/gitlab-runner@0.89.0`<br>`runix/pgadmin4@1.62.0`<br>`nfs-subdir-external-provisioner/nfs-subdir-external-provisioner@4.0.18`<br>`elastic/kibana@8.5.1` |
...
| `local-kubernetes-live` | 20 | Rendered objects were applied to Kubernetes and observed for at least one variant. |
| `render-parity` | 49 | Regular Helm and cub installer setup render-equivalent objects. |
