...
This is the `default` preset config for `istio/istiod@1.30.0`. The repo also calls this a base variant. Use this when you want to start from the chart author's normal path, with the inputs recorded.

The matching catalog page is [istio/istiod@1.30.0](https://confighub.github.io/helm-expt/site/charts/istio-istiod-1-30-0.html).

## The chart journey

We keep the Helm chart. We lock `istio/istiod@1.30.0` and choose the `default` preset config, then render it with the recorded values, namespace and release name against the recorded Kubernetes capabilities, and save the output as files.
...
The public package is `oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/istio-istiod:1.30.0@sha256:ecb9f9cbd9dc89bb7707cdc98c61797f51ddf08c155f43955d3f4372ac2e5476`. Users can pull these exact bytes without cloning this repo. When someone runs `cub installer upload`, ConfigHub stores the rendered Kubernetes YAML in a Space, where it can be searched and compared, reviewed and changed, then delivered. The example script defaults to Space `helm-istiod-default`, but users can choose another name with `CUB_SPACE=...`.
...
```sh
bash <(curl -fsSL https://confighub.github.io/helm-expt/site/sh/istio-istiod-1-30-0/default/try.sh)
```

Fast path with a ConfigHub account:

```sh
bash <(curl -fsSL https://confighub.github.io/helm-expt/site/sh/istio-istiod-1-30-0/default/confighub.sh)
```
...
| Namespace | `default` |
| Release name | `istiod` |
| Values | [`recipes/istio/istiod/1.30.0/effective-values.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/istio/istiod/1.30.0/effective-values.yaml) |
| Render intent | [`data/helm-render-intents/intents/istio-istiod-1-30-0-default.yaml`](https://github.com/confighub/helm-expt/blob/main/data/helm-render-intents/intents/istio-istiod-1-30-0-default.yaml) |
| Render variant | [`recipes/istio/istiod/1.30.0/revisions/default/r001/rendered/release-objects.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/istio/istiod/1.30.0/revisions/default/r001/rendered/release-objects.yaml) |
| Package base | [`packages/istio/istiod/1.30.0/bases/default`](https://github.com/confighub/helm-expt/tree/main/packages/istio/istiod/1.30.0/bases/default) |
| Scripts | [try.sh](https://confighub.github.io/helm-expt/site/sh/istio-istiod-1-30-0/default/try.sh) · [confighub.sh](https://confighub.github.io/helm-expt/site/sh/istio-istiod-1-30-0/default/confighub.sh) |

## Prerequisites and lifecycle steps

| When | What | How it is handled |
| --- | --- | --- |
| Before install | ClusterFeature: Namespace istio-system | kubectl create namespace istio-system --dry-run=client -o yaml \| kubectl apply -f - |
...
- Chart page: [https://confighub.github.io/helm-expt/site/charts/istio-istiod-1-30-0.html](https://confighub.github.io/helm-expt/site/charts/istio-istiod-1-30-0.html)
- Render intent: [`data/helm-render-intents/intents/istio-istiod-1-30-0-default.yaml`](https://github.com/confighub/helm-expt/blob/main/data/helm-render-intents/intents/istio-istiod-1-30-0-default.yaml)
- Rendered YAML: [`recipes/istio/istiod/1.30.0/revisions/default/r001/rendered/release-objects.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/istio/istiod/1.30.0/revisions/default/r001/rendered/release-objects.yaml)
- Package source: [`packages/istio/istiod/1.30.0/bases/default`](https://github.com/confighub/helm-expt/tree/main/packages/istio/istiod/1.30.0/bases/default)
- Generated scripts: [`site/sh/istio-istiod-1-30-0/default`](https://github.com/confighub/helm-expt/tree/main/site/sh/istio-istiod-1-30-0/default)
