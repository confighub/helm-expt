...
This is the `single-binary-filesystem` preset config for `grafana/loki@7.0.0`. The repo also calls this a base variant. Use this when you want a smaller configuration for first tests or local use.

The matching catalog page is [grafana/loki@7.0.0](https://confighub.github.io/helm-expt/site/charts/grafana-loki-7-0-0.html).

## The chart journey

We keep the Helm chart. We lock `grafana/loki@7.0.0` and choose the `single-binary-filesystem` preset config, then render it with the recorded values, namespace and release name against the recorded Kubernetes capabilities, and save the output as files.
...
The public package is `oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/grafana-loki:7.0.0@sha256:c89f127344879cdb6c7e4033735bcde8eaca3257d5c16bdcca89aad2a0f602da`. Users can pull these exact bytes without cloning this repo. When someone runs `cub installer upload`, ConfigHub stores the rendered Kubernetes YAML in a Space, where it can be searched and compared, reviewed and changed, then delivered. The example script defaults to Space `helm-loki-single-binary-filesystem`, but users can choose another name with `CUB_SPACE=...`.
...
```sh
bash <(curl -fsSL https://confighub.github.io/helm-expt/site/sh/grafana-loki-7-0-0/single-binary-filesystem/try.sh)
```

Fast path with a ConfigHub account:

```sh
bash <(curl -fsSL https://confighub.github.io/helm-expt/site/sh/grafana-loki-7-0-0/single-binary-filesystem/confighub.sh)
```
...
| Preset config | `single-binary-filesystem` |
| Namespace | `loki` |
| Release name | `loki` |
| Values | [`recipes/grafana/loki/7.0.0/effective-values.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/grafana/loki/7.0.0/effective-values.yaml) |
| Render intent | [`data/helm-render-intents/intents/grafana-loki-7-0-0-single-binary-filesystem.yaml`](https://github.com/confighub/helm-expt/blob/main/data/helm-render-intents/intents/grafana-loki-7-0-0-single-binary-filesystem.yaml) |
| Render variant | [`recipes/grafana/loki/7.0.0/revisions/single-binary-filesystem/r001/rendered/release-objects.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/grafana/loki/7.0.0/revisions/single-binary-filesystem/r001/rendered/release-objects.yaml) |
| Package base | [`packages/grafana/loki/7.0.0/bases/single-binary-filesystem`](https://github.com/confighub/helm-expt/tree/main/packages/grafana/loki/7.0.0/bases/single-binary-filesystem) |
| Scripts | [try.sh](https://confighub.github.io/helm-expt/site/sh/grafana-loki-7-0-0/single-binary-filesystem/try.sh) · [confighub.sh](https://confighub.github.io/helm-expt/site/sh/grafana-loki-7-0-0/single-binary-filesystem/confighub.sh) |
...
- Render intent: [`data/helm-render-intents/intents/grafana-loki-7-0-0-single-binary-filesystem.yaml`](https://github.com/confighub/helm-expt/blob/main/data/helm-render-intents/intents/grafana-loki-7-0-0-single-binary-filesystem.yaml)
- Rendered YAML: [`recipes/grafana/loki/7.0.0/revisions/single-binary-filesystem/r001/rendered/release-objects.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/grafana/loki/7.0.0/revisions/single-binary-filesystem/r001/rendered/release-objects.yaml)
- Package source: [`packages/grafana/loki/7.0.0/bases/single-binary-filesystem`](https://github.com/confighub/helm-expt/tree/main/packages/grafana/loki/7.0.0/bases/single-binary-filesystem)
- Generated scripts: [`site/sh/grafana-loki-7-0-0/single-binary-filesystem`](https://github.com/confighub/helm-expt/tree/main/site/sh/grafana-loki-7-0-0/single-binary-filesystem)
