...
We keep the Helm chart. We lock `prometheus-community/prometheus-adapter@5.3.0` and choose the `default` preset config, then render it with the recorded values, namespace and release name against the recorded Kubernetes capabilities, and save the output as files.
