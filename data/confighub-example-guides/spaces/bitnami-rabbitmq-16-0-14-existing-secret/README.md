...
We keep the Helm chart. We lock `bitnami/rabbitmq@16.0.14` and choose the `existing-secret` preset config, then render it with the recorded values, namespace and release name against the recorded Kubernetes capabilities, and save the output as files.
