...
This is the `ha` preset config for `bitnami/spark@10.0.3`. The repo also calls this a base variant. Use this when you want the reviewed high-availability shape instead of the simplest default.
...
We keep the Helm chart. We lock `bitnami/spark@10.0.3`, choose the `ha` preset config, render it with the recorded values, namespace, release name, and Kubernetes capabilities, then save the output as files.
...
The public package is `oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/bitnami-spark:10.0.3`. Users can pull it without cloning this repo. When someone runs `cub installer upload`, ConfigHub stores the rendered Kubernetes YAML in a Space so it can be searched, compared, reviewed, changed, and delivered. The example script defaults to Space `helm-spark-ha`, but users can choose another name with `CUB_SPACE=...`.
...
```sh
bash <(curl -fsSL https://confighub.github.io/helm-expt/site/sh/bitnami-spark-10-0-3-ha/try.sh)
```

Fast path with a ConfigHub account:

```sh
bash <(curl -fsSL https://confighub.github.io/helm-expt/site/sh/bitnami-spark-10-0-3-ha/confighub.sh)
```
...
| Namespace | `default` |
| Release name | `spark` |
| Values | [`recipes/bitnami/spark/10.0.3/effective-values-ha.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/bitnami/spark/10.0.3/effective-values-ha.yaml) |
| Render intent | [`data/helm-render-intents/intents/bitnami-spark-10-0-3-ha.yaml`](https://github.com/confighub/helm-expt/blob/main/data/helm-render-intents/intents/bitnami-spark-10-0-3-ha.yaml) |
| Render variant | [`recipes/bitnami/spark/10.0.3/revisions/ha/r001/rendered/release-objects.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/bitnami/spark/10.0.3/revisions/ha/r001/rendered/release-objects.yaml) |
| Package base | [`packages/bitnami/spark/10.0.3/bases/ha`](https://github.com/confighub/helm-expt/tree/main/packages/bitnami/spark/10.0.3/bases/ha) |
| Scripts | [try.sh](https://confighub.github.io/helm-expt/site/sh/bitnami-spark-10-0-3-ha/try.sh) · [confighub.sh](https://confighub.github.io/helm-expt/site/sh/bitnami-spark-10-0-3-ha/confighub.sh) |
...
- Render intent: [`data/helm-render-intents/intents/bitnami-spark-10-0-3-ha.yaml`](https://github.com/confighub/helm-expt/blob/main/data/helm-render-intents/intents/bitnami-spark-10-0-3-ha.yaml)
- Rendered YAML: [`recipes/bitnami/spark/10.0.3/revisions/ha/r001/rendered/release-objects.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/bitnami/spark/10.0.3/revisions/ha/r001/rendered/release-objects.yaml)
- Package source: [`packages/bitnami/spark/10.0.3/bases/ha`](https://github.com/confighub/helm-expt/tree/main/packages/bitnami/spark/10.0.3/bases/ha)
- Generated scripts: [`site/sh/bitnami-spark-10-0-3-ha`](https://github.com/confighub/helm-expt/tree/main/site/sh/bitnami-spark-10-0-3-ha)
