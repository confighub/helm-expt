...
This is the `default` preset config for `nats/nats@2.14.0`. The repo also calls this a base variant. Use this when you want to start from the chart author's normal path, with the inputs recorded.

The matching catalog page is [nats/nats@2.14.0](https://confighub.github.io/helm-expt/site/charts/nats-nats-2-14-0.html).

## The chart journey

We keep the Helm chart. We lock `nats/nats@2.14.0` and choose the `default` preset config, then render it with the recorded values, namespace and release name against the recorded Kubernetes capabilities, and save the output as files.
...
The public package is `oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/nats-nats:2.14.0@sha256:bf8546bb6289a3c325e6396b2da6c03f6cac3961652aefbed116063d588e745b`. Users can pull these exact bytes without cloning this repo. When someone runs `cub installer upload`, ConfigHub stores the rendered Kubernetes YAML in a Space, where it can be searched and compared, reviewed and changed, then delivered. The example script defaults to Space `helm-nats-default`, but users can choose another name with `CUB_SPACE=...`.
...
```sh
cub installer setup --pull oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/nats-nats:2.14.0@sha256:bf8546bb6289a3c325e6396b2da6c03f6cac3961652aefbed116063d588e745b --base default --work-dir ./nats-nats-2-14-0-default --non-interactive --namespace default
```
...
| Namespace | `default` |
| Release name | `nats` |
| Values | [`recipes/nats/nats/2.14.0/effective-values.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/nats/nats/2.14.0/effective-values.yaml) |
