...
# percona/pg-operator 3.0.0 - no-crds
...
This is the `no-crds` preset config for `percona/pg-operator@3.0.0`. The repo also calls this a base variant. Use this when your platform, GitOps bootstrap, or another chart owns the CRDs.
...
We keep the Helm chart. We lock `percona/pg-operator@3.0.0` and choose the `no-crds` preset config, then render it with the recorded values, namespace and release name against the recorded Kubernetes capabilities, and save the output as files.
...
The public package is `oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/percona-pg-operator:3.0.0@sha256:c3afc463cdf2fe0d978fd0f2513dab3e03279dafbd1827bcb8013cd692173db1`. Users can pull these exact bytes without cloning this repo. When someone runs `cub installer upload`, ConfigHub stores the rendered Kubernetes YAML in a Space, where it can be searched and compared, reviewed and changed, then delivered. The example script defaults to Space `helm-pg-operator-no-crds`, but users can choose another name with `CUB_SPACE=...`.
...
```sh
cub installer setup --pull oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/percona-pg-operator:3.0.0@sha256:c3afc463cdf2fe0d978fd0f2513dab3e03279dafbd1827bcb8013cd692173db1 --base no-crds --work-dir ./percona-pg-operator-3-0-0-no-crds --non-interactive --namespace default
```
...
| Values | [`recipes/percona/pg-operator/3.0.0/effective-values-no-crds.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/percona/pg-operator/3.0.0/effective-values-no-crds.yaml) |
