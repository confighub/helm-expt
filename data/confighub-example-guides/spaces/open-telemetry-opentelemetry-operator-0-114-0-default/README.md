...
This is the `default` preset config for `open-telemetry/opentelemetry-operator@0.114.0`. The repo also calls this a base variant. Use this when you want to start from the chart author's normal path, with the inputs recorded.
...
We keep the Helm chart. We lock `open-telemetry/opentelemetry-operator@0.114.0`, choose the `default` preset config, render it with the recorded values, namespace, release name, and Kubernetes capabilities, then save the output as files.

That captured output is the render variant: [`recipes/open-telemetry/opentelemetry-operator/0.114.0/revisions/default/r001/rendered/release-objects.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/open-telemetry/opentelemetry-operator/0.114.0/revisions/default/r001/rendered/release-objects.yaml). It contains 17 Kubernetes object(s): CustomResourceDefinition x4, ClusterRole x2, Service x2, Certificate x1, ClusterRoleBinding x1, Deployment x1, Issuer x1, MutatingWebhookConfiguration x1.

The public package is `oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/open-telemetry-opentelemetry-operator:0.114.0@sha256:dacf369ea222ff6e4b2538c39c5743867a140991f1b724e0d7e59667d1892a5a`. Users can pull these exact bytes without cloning this repo. When someone runs `cub installer upload`, ConfigHub stores the rendered Kubernetes YAML in a Space so it can be searched, compared, reviewed, changed, and delivered. The example script defaults to Space `helm-opentelemetry-operator-default`, but users can choose another name with `CUB_SPACE=...`.
...
```sh
cub installer setup --pull oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/open-telemetry-opentelemetry-operator:0.114.0@sha256:dacf369ea222ff6e4b2538c39c5743867a140991f1b724e0d7e59667d1892a5a --base default --work-dir ./open-telemetry-opentelemetry-operator-0-114-0-default --non-interactive --namespace default
```
...
| Values | [`recipes/open-telemetry/opentelemetry-operator/0.114.0/effective-values.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/open-telemetry/opentelemetry-operator/0.114.0/effective-values.yaml) |
