...
We keep the Helm chart. We lock `bitnami/postgresql@18.6.7` and choose the `existing-secret` preset config, then render it with the recorded values, namespace and release name against the recorded Kubernetes capabilities, and save the output as files.
