...
# traefik/traefik 40.2.0 - no-crds
...
This is the `no-crds` preset config for `traefik/traefik@40.2.0`. The repo also calls this a base variant. Use this when your platform, GitOps bootstrap, or another chart owns the CRDs.
...
We keep the Helm chart. We lock `traefik/traefik@40.2.0` and choose the `no-crds` preset config, then render it with the recorded values, namespace and release name against the recorded Kubernetes capabilities, and save the output as files.
...
The public package is `oci://europe-west1-docker.pkg.dev/nth-fort-499605-q5/helm-expt/traefik-traefik:40.2.0@sha256:b127b8df699116a6e6252bcf5a1199c2a32640b7e99de2e45fd507dd29022535`. Users can pull these exact bytes without cloning this repo. When someone runs `cub installer upload`, ConfigHub stores the rendered Kubernetes YAML in a Space, where it can be searched and compared, reviewed and changed, then delivered. The example script defaults to Space `helm-traefik-no-crds`, but users can choose another name with `CUB_SPACE=...`.
...
```sh
bash <(curl -fsSL https://confighub.github.io/helm-expt/site/sh/traefik-traefik-40-2-0/no-crds/try.sh)
```

Fast path with a ConfigHub account:

```sh
bash <(curl -fsSL https://confighub.github.io/helm-expt/site/sh/traefik-traefik-40-2-0/no-crds/confighub.sh)
```
...
| Values | [`recipes/traefik/traefik/40.2.0/effective-values-no-crds.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/traefik/traefik/40.2.0/effective-values-no-crds.yaml) |
| Render intent | [`data/helm-render-intents/intents/traefik-traefik-40-2-0-no-crds.yaml`](https://github.com/confighub/helm-expt/blob/main/data/helm-render-intents/intents/traefik-traefik-40-2-0-no-crds.yaml) |
| Render variant | [`recipes/traefik/traefik/40.2.0/revisions/no-crds/r001/rendered/release-objects.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/traefik/traefik/40.2.0/revisions/no-crds/r001/rendered/release-objects.yaml) |
| Package base | [`packages/traefik/traefik/40.2.0/bases/no-crds`](https://github.com/confighub/helm-expt/tree/main/packages/traefik/traefik/40.2.0/bases/no-crds) |
| Scripts | [try.sh](https://confighub.github.io/helm-expt/site/sh/traefik-traefik-40-2-0/no-crds/try.sh) · [confighub.sh](https://confighub.github.io/helm-expt/site/sh/traefik-traefik-40-2-0/no-crds/confighub.sh) |
...
- Render intent: [`data/helm-render-intents/intents/traefik-traefik-40-2-0-no-crds.yaml`](https://github.com/confighub/helm-expt/blob/main/data/helm-render-intents/intents/traefik-traefik-40-2-0-no-crds.yaml)
- Rendered YAML: [`recipes/traefik/traefik/40.2.0/revisions/no-crds/r001/rendered/release-objects.yaml`](https://github.com/confighub/helm-expt/blob/main/recipes/traefik/traefik/40.2.0/revisions/no-crds/r001/rendered/release-objects.yaml)
- Package source: [`packages/traefik/traefik/40.2.0/bases/no-crds`](https://github.com/confighub/helm-expt/tree/main/packages/traefik/traefik/40.2.0/bases/no-crds)
- Generated scripts: [`site/sh/traefik-traefik-40-2-0/no-crds`](https://github.com/confighub/helm-expt/tree/main/site/sh/traefik-traefik-40-2-0/no-crds)
