...
| What do I have? | `aicr` | `inspection` | no | yes | no | `completed` | `available` | AICR snapshot and diff report observed differences between GPU nodes without selecting a recipe or deploying a bundle. To decide whether a difference is a fault, compare each node with the provider-curated source variant intended for its service, accelerator, operating system, workload intent, platform, and relevant hardware. |
