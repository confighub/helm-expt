...
base="${KPS_LIFECYCLE_BASE:-minimal}"
...
if [[ "$base" != "minimal" ]]; then
  printf 'This packaged route supports the minimal base, not %s.\n' "$base" >&2
  exit 1
fi
...
metadata:
  name: prometheus-community-kube-prometheus-stack-87-19-2-admission
spec:
  result: pass
